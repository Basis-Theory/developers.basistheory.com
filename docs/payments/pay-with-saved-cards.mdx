...
Once the customer selected the saved card, invoke the PSP or Acquirer API to process the transaction, passing:
- The Basis Theory card token identifier, where the card details are expected;
