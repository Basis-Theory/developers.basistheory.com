...
sequenceDiagram
  ...
  participant BasisTheory as Basis Theory
  participant AFS as Anti-Fraud System
	participant PSP as Payment Processor
  Consumer ->> Merchant: Enters checkout page
  Merchant ->> Merchant: Loads AFS SDK
  Merchant ->> Merchant: Loads Basis Theory SDK
	Consumer ->> BasisTheory: Sends card data
  Merchant ->> BasisTheory: Sends Device and Transaction Info
  BasisTheory ->> AFS: Verifies Order
	AFS ->> BasisTheory: Returns Decision
  BasisTheory ->> Merchant: Returns Decision and Token ID
  Merchant ->> PSP: Process Transaction
```
...
sequenceDiagram
  ...
  participant BasisTheory as Basis Theory
  participant PVS
	Consumer ->> Merchant: Inputs personal information
	Consumer ->> BasisTheory: Sends card data
	Merchant ->> BasisTheory: Sends customer information
	BasisTheory ->> PVS: Check Payment Information
  PVS ->> BasisTheory: Verification Result
  BasisTheory ->> Merchant: Verification Result
```
