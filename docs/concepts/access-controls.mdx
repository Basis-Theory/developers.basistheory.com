...
Basis Theory secures sensitive data as [Tokens](/docs/concepts/what-are-tokens) that are stored within an isolated compliant environment, called a [Tenant](#what-are-tenants). Your systems interact with Tokens or Services (e.g. Proxy, Reactors) through the Basis Theory API using an API key that is issued to an [Application](#what-are-applications). Access to your Tenant's data is limited by applying access controls to an Application via [Permissions](/docs/api/applications/permissions) and [Access Rules](#what-are-access-rules) to ensure services and users only have access to the data they need.
...
<Alert>Basis Theory supports Single Sign-On (SSO) for simplified access management. If you are interested in using it, please [reach out to us](https://basistheory.com/contact?how_can_we_help_=I+want+to+use+SSO+to+manage+Basis+Theory+accounts.).</Alert>

Basis Theory accounts are usually created after one of the following patterns:

- 🧑‍💻 **Personal Accounts** (e.g. _john.doe@<span>yourcompany.com</span>_) are used in local development situations such as learning, testing, contributing, POC, etc.
- 🏢 **Managed Accounts** (e.g. _payments@<span>yourcompany.com</span>_, _platform@<span>yourcompany.com</span>_) are team or company-owned accounts used to manage environments, such as Shared Development, Staging and Production.

See below an example of an optimal Account and Tenant setup for a cross-region deployment:

[![Account x Tenant Setup](/img/concepts/account_tenant_setup.png)](/img/concepts/account_tenant_setup.png)
...
Everything starts with a Tenant in your Basis Theory account. Tenants represent an isolated environment in which your [Tokens](/docs/concepts/what-are-tokens) and other resources are defined, managed, and stored. Data is not shared between Tenants, but you're allowed to create as many Tenants as you'd like within Basis Theory.

There are two types of Tenants:

1. 🛠️ **Testing** Tenants are used for everything non-production, and should only be used to store synthetic data.
...
<Alert> 🌍 With Basis Theory deployable stack, new region deployments are rapidly provisioned. If you are interested in one that is not listed, please [reach out to us](https://basistheory.com/contact?how_can_we_help_=I+am+interested+in+a+region-specific+deployment.+&lt;INSERT+REGION&gt;).</Alert>

The user account that was used to create a Tenant in the [Portal](https://portal.basistheory.com) will be designated as the Tenant Owner. New members can be invited to an existing Tenant through the [Members](https://portal.basistheory.com/members) tab. Managing Tenant Members can also be done directly through the [API](/docs/api/tenants/tenant-members).
