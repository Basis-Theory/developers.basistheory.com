...
You can learn more about implementing the session authorization flow in the guide [Access Data Using Sessions](/docs/guides/govern/sessions) or
continue reading below to learn about why you might consider using sessions.

### Why Use Sessions?

Sessions can help you achieve improved security by limiting the access granted to a public application and its API key.
Since public API keys are publicly exposed within frontend applications, they can potentially be accessed by untrusted parties
who inspect or decompile your web or mobile application source code, or by inspecting network traffic. Because of these risks,
Basis Theory only allows public applications to be granted permission to create or update tokens, and never to read or use tokens.
...
- User-Specific Access Controls: Apply your own custom authorization logic to limit access to only the tokens that should be accessible by each user
...
beyond only collecting data within your frontend applications.

Sessions are a good fit for the following use cases:

- Revealing tokens in a front end application in plain text (see [reveal elements](/docs/sdks/web/javascript#reveal-sensitive-data))
- Calling the proxy or reactors from your frontend application
- Restricting the access granted to public API keys to prevent unauthorized use of exposed API keys
