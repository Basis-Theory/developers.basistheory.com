...
<Alert> 🌍 With Basis Theory deployable stack, new region deployments are rapidly provisioned. If you are interested in one that is not listed above, please [reach out to us](https://basistheory.com/contact?how_can_we_help_=I+am+interested+in+a+region-specific+deployment.+&lt;INSERT+REGION&gt;).</Alert>

The user account used to create a Tenant in the [Portal](https://portal.basistheory.com) is the designated Tenant Owner. New members can be invited to an existing Tenant through the [Members](https://portal.basistheory.com/members) tab. Managing Tenant Members can also be done directly through the [API](/docs/api/tenants/tenant-members).
