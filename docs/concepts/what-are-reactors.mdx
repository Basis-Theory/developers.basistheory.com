...
To retrieve the result of the asynchronous invocation, make a [request](/docs/api/reactors#retrieve-result) to the Retrieve Result endpoint using the reactor and request ID provided in the webhook. This request ensures the system receiving the webhook has access to the results of the reactor.
