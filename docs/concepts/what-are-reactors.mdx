...
a `reactor.failed` webhook containing a [Reactor Runtime Error](/docs/api/reactors/invoke-async-reactors#reactor-error-codes) indicating that a timeout occurred.
...
Any errors that occur while executing the reactor will be returned from the same result endpoint. See [Reactor Errors](/docs/api/reactors/invoke-async-reactors#reactor-error-codes) for details.
...
Your reactor code will then be asynchronously executed in our serverless compute environment for up to 5 minutes. If the timeout period is exceeded, the reactor will be terminated and you will receive
a callback containing a [Reactor Runtime Error](/docs/api/reactors/invoke-async-reactors#reactor-error-codes) indicating that a timeout occurred.
...
Any errors that occur while executing the reactor will be delivered to the `callback_url`. See [Reactor Errors](/docs/api/reactors/invoke-async-reactors#reactor-error-codes) for details.
