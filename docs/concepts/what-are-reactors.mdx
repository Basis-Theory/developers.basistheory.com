...
Reactors may be invoked by an Application with `token:use` permission, which enables the Reactor to [detokenize](/docs/expressions/detokenization) tokens
...
The reactor will perform some synchronous request validation on your request args to ensure the request is valid,
then immediately respond with an empty HTTP response with `202 Accepted` status code.

Your reactor code will be asynchronously executed in our serverless compute environment for up to the configured timeout specified in the `timeout_ms`
parameter (if not specified, this defaults to `10000` or 10 seconds). If the timeout period is exceeded, the reactor will be terminated and you will receive
a callback containing a [Reactor Runtime Error](/docs/api/reactors/reactor-errors).
...
respond with a `2xx` status code, delivery will retried up to 10 times with exponential backoff.
