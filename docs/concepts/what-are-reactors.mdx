...
Once the reactor is completed, the `reactor.completed-async`  [webhook event](/docs/api/webhooks/eventdata#reactor-completed-asynchronously) will be raised.
To retrieve the result of the asynchronous invocation, make a [request](/docs/api/reactors#retrieve-result) to the Retrieve Result using the reactor and request ID provided in the webhook.
