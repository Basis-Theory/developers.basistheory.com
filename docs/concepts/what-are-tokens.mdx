...
When deduplicated, the data and metadata of an existing token can be updated if the [token deduplication behavior](/docs/api/tenants#tenant-settings-object) setting is set to Update Reading. Otherwise, they remain unchanged. [Token Intents](/docs/concepts/token-intents) are always deduplicated with the Update Reading behavior.
