...
In the following example show how to use our default fingerprinting capabilities on a Card token, and a custom fingerprint expression on a Generic Token.

<Tabs className="bt-tabs">
  <TabItem value="card" label="Card Example">
    Card token's automatically fingerprint on the card number:
