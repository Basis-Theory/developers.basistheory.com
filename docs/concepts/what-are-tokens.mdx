...
<Alert type={Alerts.WARNING}>
  In order to maintain PCI compliance while capturing credit cards, you will
  need to use Basis Theory's <code>card</code> token type or store the CVC in a
  separate
  <code>token</code> while setting <code>expires_at</code> TTL property.
</Alert>
