...
When a token is created, it will be counted as a MAT for the current month. If the [tokenize](/docs/api/tokens/tokenize)
...
- Retrieving a single [token by id](/docs/api/tokens#get-a-token) - the returned token will be counted as a MAT.
- Retrieving a [list of tokens](/docs/api/tokens#list-tokens) - any tokens returned from the list API will be counted as MATs.
- [Searching tokens](/docs/api/tokens/search) - any tokens returned from the search API will be counted as MATs.
...
A token is only counted as a Monthly Active Token if it has been interacted with through the Basis Theory API. Tokens that are viewed or interacted with through the Portal are not counted as MATs.
...
Likewise, [detokenizing](/docs/expressions/detokenization) a single token multiple times within a Reactor or Proxy request,
for example to select multiple properties from a token using a [JSON Path](/docs/expressions/filters#json) transformation,
...
Are you interested in using Basis Theory in production? Please [contact us](https://basistheory.com/contact) to discuss a Custom plan with volume discount pricing tailored to your organization's needs.
:::
