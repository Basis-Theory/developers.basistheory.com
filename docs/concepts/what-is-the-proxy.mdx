...
domain like `https://secure.yourdomain.com` instead of the default proxy domain `https://api.basistheory.com/proxy` to invoke your inbound proxy.
All you would need to do is the following:

1. Configure a Basis Theory proxy instance
2. Have a domain you own with a valid SSL certificate
3. After we register your domain with our DNS provider, you'll need a CNAME record pointing to `btproxy.cloud` and a new TXT record with a value
we'll provide you for validation.

<Alert>
  If you're ready to get started with white labeling your Basis Theory proxies, <a href="https://basistheory.com/contact">
  please reach out</a>.
</Alert>
