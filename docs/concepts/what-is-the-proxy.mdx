...
The request is transformed by evaluating any [expressions](/docs/expressions) included in the request (patterns of the form `{{<tokenId>}}`, where `<tokenId>` is the id of a token created within your tenant). Any detokenization expressions will be replaced with the detokenized `data` value of this token.
...
For further details about how to use the Proxy, check out [our docs](/docs/api/proxies/invoke-proxy).
...
[Use Token Data in HTTP Requests](https://developers.basistheory.com/guides/use-token-data-in-http-requests/)
