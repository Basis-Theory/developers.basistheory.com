...
1. **[Ephemeral Proxy](/docs/api/proxies/invoke-ephemeral-proxies):**
...
Your system initiates an outbound HTTP request to the [Ephemeral Proxy](/docs/api/proxies/invoke-ephemeral-proxies)
...
For further details, check out [our API docs](/docs/api/proxies/pre-configured-proxies).

### Payment Processors

Any payment processor is supported by the Basis Theory Proxy, as long as they expose an API endpoint that accepts cardholder data (e.g., PAN, expiration date, CVC). A few technical and compliance conditions may apply:

1. **IP Whitelisting**: Some processors enforce IP restrictions and may require you to whitelist [Basis Theory’s outbound IP addresses]((/docs/api/ip-addresses).

2. **Proof of Compliance**: Many processors will request proof of PCI compliance before enabling these endpoints in your account. This step typically takes a few days once you share Basis Theory’s Attestation of Compliance from our [Trust Center](https://trust.basistheory.com).

3. **Testing Support**: Most processors maintain a set of test scenarios using test card numbers. Basis Theory can tokenize any Luhn-valid card number, which enables testing against processor requirements. We also provide our own [test cards](/docs/api/testing) for scenario-testing Basis Theory features — these should be tested independently.

4. **Response Data Handling**: In rare cases, a processor’s API response may include sensitive data. You can redact or tokenize this data using a [Pre-configured Proxy Response Transform](/docs/api/proxies/pre-configured-proxies#response-transforms).
