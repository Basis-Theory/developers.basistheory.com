...
In general, filters are applied during detokenization using one of the following syntax:
...
The following is how you would use the `card_brand` from the [BIN Details Enrichment](http://localhost:3000/docs/api/tokens/token-enrichments#bin-details-) and the name from `metadata`:
