...
ApplicationKey key = client.applicationKeys().create(ApplicationKey.builder()
    .name("My Key")
    .type("private")
...
```java showLineNumbers
List<ApplicationKey> keys = client.applicationKeys().list("id");
```
...
```java showLineNumbers
ApplicationKey key = client.applicationKeys().get("id", "keyId");
```
