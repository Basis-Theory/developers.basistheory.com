...
const bt = await new BasisTheory().init("<Public API Key>");
...
var client = new SessionClient("<Public API Key>");
...
with basistheory.ApiClient(configuration=basistheory.Configuration(api_key="<Public API Key>")) as api_client:
