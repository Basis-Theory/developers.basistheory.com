...
metadata.put("nonSensitiveField", "Non-Sensitive Value");

Application application = client.applications().create(Application.builder()
    .name("My Application")
    .type("private")
    .metadata(metadata)
    .build());
...
```java showLineNumbers
List<Application> applications = client.applications().list();
```
...
```java showLineNumbers
Application application = client.applications().get("id");
```
...
```java showLineNumbers
Application application = client.applications().getByKey();
```
...
```java showLineNumbers
Application updatedApplication = client.applications().update("id", Application.builder()
    .name("My Example App")
