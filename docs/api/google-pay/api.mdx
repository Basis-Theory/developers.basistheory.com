...
Decrypts a Google Pay token and creates either a Google Pay token or a Basis Theory's [Token Intent](/docs/api/tokens/token-intents#token-intent-object) based on the provided ecrypted token. This endpoint only supports the `ECv2` token versions. It supports both auth methods, `PAN_ONLY` and `CRYPTOGRAM_3DS`.
...
<Alert type={Alerts.Information}>
  For encrypted cards that consist of PAN a Token Intent will be created. Refer to Basis Theory's [Token Intents documentation](/docs/api/tokens/token-intents#token-intent-object) for more information.
</Alert>

## Retrieve

Retrieves an already tokenized Google Pay token. For encrypted cards that consist of PAN look at Basis Theory's [Token Intents documentation](/docs/api/tokens/token-intents#get-a-token-intent).
...
Deletes an already tokenized Google Pay token. For encrypted cards that consist of PAN look at Basis Theory's [Token Intents documentation](/docs/api/tokens/token-intents#delete-token-intent).
