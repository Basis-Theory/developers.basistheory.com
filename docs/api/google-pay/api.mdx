...
Decrypts a [Google Pay™](https://developers.google.com/pay/api) payment method token and creates either a Google Pay™ resource or a Basis Theory's [Token Intent](/docs/api/tokens/token-intents#token-intent-object) based on the provided encrypted token. This endpoint only supports the `ECv2` token versions. It supports both auth methods, `PAN_ONLY` and `CRYPTOGRAM_3DS`.
...
|---------------|----------------------------------------------------------------|
| [PAN_ONLY](https://developers.google.com/pay/api/web/guides/resources/payment-data-cryptography#pan_only) | [`token_intent`](/docs/api/tokens/token-types#card)                    |
| [CRYPTOGRAM_3DS](https://developers.google.com/pay/api/web/guides/resources/payment-data-cryptography#cryptogram_3ds)  | [`google_pay`](/docs/api/google-pay/api#response)  |
...
| `google_payment_data` | true | The Google Pay™ [resource](https://developers.google.com/pay/api/web/guides/resources/payment-data-cryptography#payment-method-token-structure). |

### Response

Returns a Google Pay™ resource for tokenized cards that consists of device PAN and cryptograms.
The return will include the Card BIN enrichments.
Returns [an error](/docs/api/errors) if there were validation errors, if the Google Pay™ payment method token failed to decrypt or the token failed to create.
...
Retrieves an already tokenized Google Pay™ resource. For encrypted cards with an authorization method of "PAN_ONLY" use [Token Intents documentation](/docs/api/tokens/token-intents#get-a-token-intent).
...
Deletes an already tokenized Google Pay™ resource. For encrypted cards with an authorization method of "PAN_ONLY" use [Token Intents documentation](/docs/api/tokens/token-intents#delete-token-intent).
