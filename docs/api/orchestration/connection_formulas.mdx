...
Adyen provides a self-service sign-up for a sandbox account; navigate to [the sign-up](https://www.adyen.com/signup) page to get started.
...
3. [Request access from Adyen](https://ca-live.adyen.com/ca/ca/contactUs/support.shtml) to add the `API PCI Payments` role on the credential created in step 2. (Find more information from [Adyen](https://help.adyen.com/knowledge/ecommerce-integrations/error-codes/why-do-i-receive-a-403-or-010-not-allowed-error))
4. You'll now use this credential into the Basis Theory Connection configuration.
...
- A `merchant account` is the public ID of your account used when connecting to Adyen.
