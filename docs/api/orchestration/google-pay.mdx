...
This endpoint is able to be used with both `TEST` and `PRODUCTION` Google Pay tokens. To use a Google Pay `TEST` token, set the [`TEST` environment](https://developers.google.com/pay/api/web/guides/tutorial#js-load) when generating the Google Pay token and use a Basis Theory Application under a [`TEST` Tenant](/docs/concepts/access-controls#what-are-tenants) for making your `/connections/google-pay/tokenize` request. `PROD` Tenants will only work with `PRODUCTION` Google Pay tokens.
