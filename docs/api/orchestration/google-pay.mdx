...
There are no requirements for the value of `gatewayMerchantId`.
```javascript
...
Basis Theory provides an mechanism to create fake token intents using this endpoint.
...
The `signedMessage` should contain `nubmer`, `expiration_month`, and `expiration_year`.
