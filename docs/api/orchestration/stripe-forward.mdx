...
This endpoint differs from the [Create Token](/docs/api/tokens/#create-token) endpoint by conforming to Stripe's Forwarding API contract: `card` contains Stripe's payment method details, and `metadata` contains any Basis Theory specific token attributes that were specified when calling the Forwarding API.
