...
To achieve this, this endpoint is different from the normal [Create token](/docs/api/tokens/#create-token) endpoint since it receives the card details
under the card field, and all the Basis Theory specific attributes under the metadata object, which Stripes puts any
additional data and forwards it. The nested metadata is Basis Theory's token metadata, whilst the root level one is Stripe's.
...
<Tabs className="bt-tabs" groupId="languages">
  ...
  <TabItem value="javascript" label="JavaScript">
    ...
    const token = await bt.connections.stripeForwardTokenize({
      card: {
      number: "4242424242424242",
      exp_month: "07",
      exp_year: "2030"
      ...
      searchIndexes: ["{{ data.number | last4 }}"],
  ...
  <TabItem value="csharp" label="C#">
    ...
    using BasisTheory.net.Connections;
    using BasisTheory.net.Connections.Requests;

    var client = new ConnectionsClient("<API_KEY>");

    var token = await client.StripeForwardTokenize(new StripeTokenizeRequest {
      Card = new {
        number = "4242424242424242",
        exp_month = "07",
        exp_year = "2030"
      ...
      Containers = new List<string> { "/pci/high/" },
      Metadata = new Dictionary<string, string>
    {
    { "nonSensitiveField",  "Non-Sensitive Value" }
    },
      SearchIndexes = new List<string> { "{{ data.number | last4 }}" },
      FingerprintExpression = "{{ data.number }}",
      DeduplicateToken = true,
  ...
  <TabItem value="java" label="Java">
    ...
    StripeTokenizeRequest stripeTokenizeRequest = new StripeTokenizeRequest();
    CreateTokenResponse result = apiInstance.stripeForwardTokenize(stripeTokenizeRequest
    .card(Map.of(
      "number", "4242424242424242",
      "exp_month", "07",
      "exp_year", "2030"
    ...
    "searchIndexes", Arrays.asList("{{ data.number | last4 }}")
    "fingerprintExpression", "{{ data.number }}"
    "deduplicateToken", true
    "expiresAt", "8/26/2030 7:23:57 PM -07:00")
    )
  ...
  <TabItem value="python" label="Python">
    ...
    import basistheory
    from basistheory.api import connections_api
    from basistheory.model.stripe_tokenize_request import StripeTokenizeRequest

    with basistheory.ApiClient(configuration=basistheory.Configuration(api_key="<API_KEY>")) as api_client:
    connections_client = connections_api.StripeForwardApi(api_client)

    token = connections_client.create(stripe_tokenize_request=StripeTokenizeRequest(
    card={
      "number": "4242424242424242",
      "exp_month": "07",
      "exp_year": "2030"
  ...
  <TabItem value="go" label="Go">
    ...
    stripeTokenizeRequest := *basistheory.NewStripeTokenizeRequest()
    stripeTokenizeRequest.SetCard(map[string]interface{}{
      "number":           "4242424242424242",
      "exp_month": 12,
      "exp_year":  2025,
      "cvc": "123",
    ...
    stripeTokenizeRequest.SetMetadata(map[string]interface{}{
    ...
    "search_indexes": []string{"{{ data.number | last4 }}"},
    "fingerprint_expression": "{{ data.number }}",
    "deduplicate_token": true,
    ...
    createTokenResponse, createTokenHttpResponse, createErr := apiClient.ConnectionsApi.StripeForwardTokenize(contextWithAPIKey).StripeTokenizeRequest(stripeTokenizeRequest).Execute()
  }
    ```
