...
<Alert type={Alerts.WARNING}>
    When using a Token ID or Token Intent ID, the record must be a card token containing at least number, expiration month and expiration year.
</Alert>
