...
Returns a [paginated object](/docs/api/pagination#cursor-pagination-object) with the `data` property containing an array of [logs](#log-object). Providing any query parameters will filter the results. Returns [an error](/docs/api/errors) if logs could not be retrieved.
