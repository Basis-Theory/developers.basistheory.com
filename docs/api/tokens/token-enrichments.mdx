...
Card details include details specific details on the card to enable better visibility into the card's origin and function. These values are derived from BIN Details to ensure accuracy of the information and also making the BIN Details enrichment a pre-requisite for this enrichment.
