...
When a specific Enrichment is activated for a Tenant, each is added as a token property and returned during every [token operation](/docs/api/tokens/) and [Detokenization Expressions](/docs/expressions/detokenization#using-token-properties).
