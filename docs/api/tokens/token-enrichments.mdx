...
When a specific Enrichment is activated for a Tenant - they are returned on every token operation and are usable as a token property in all [Detokenization Expressions](/docs/expressions/detokenization#using-token-properties).
