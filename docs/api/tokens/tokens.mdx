...
Token token = client.tokens().create(Token.builder()
    .type("token")
    .data("Sensitive Value")
    .mask("{{ data | reveal_last: 4 }}")
...
```java showLineNumbers
Page<Token> page = client.tokens().listV2();
```
...
```java showLineNumbers
Token token = client.tokens().get("id");
```
...
```java showLineNumbers
Token updatedToken = client.tokens().update("id", Token.builder()
    .data("Sensitive Value")
    .mask("{{ data | reveal_last: 4 }}")
    .containers(Arrays.asList("/general/high/"))
    .metadata(new HashMap<String, String>() {{
