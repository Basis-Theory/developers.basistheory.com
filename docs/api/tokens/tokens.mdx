...
<HttpMethod
  ...
  endpoint="https://api.basistheory.com/tokens"
/>
...
Get a list of tokens for the Tenant, supporting basic search criteria. If you need to perform a more advanced token search, see [Search Tokens](/docs/api/tokens/search).
...
<HttpMethod
  method={HttpMethods.GET}
  endpoint="https://api.basistheory.com/tokens/{id}"
/>
...
The `cvc` property automatically expires and is deleted after one hour in order to comply with PCI requirements.
