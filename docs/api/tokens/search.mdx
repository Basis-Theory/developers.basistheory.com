...
Token search supports a [Lucene](http://www.lucenetutorial.com/lucene-query-syntax.html)-based query syntax to build complex search criteria.
...
Basis Theory supports search across sensitive token data by creating and securely storing search indexes at the time of token creation.
