...
<Alert type={Alerts.ERROR}>Multiple Terms in a single `query` is deprecated and support will be removed no later than March 1st, 2025.</Alert>
...
<Alert>
  ...
  Combining terms for multiple <a href="#searchable-token-fields">token fields</a> in the same query is not supported.
