...
The `token:read` permission must be granted on each token container, and the required scope of this permission
is determined by the [containers](/docs/concepts/what-are-containers) of those tokens being detokenized in the request.
