...
import CardDetails from "@site/docs/api/tokens/_bank_details.mdx";
...
tokenIntentsClient.get("665bf471-d675-4165-9336-4fc9c1a73fad");
```
  </TabItem>
...
```go showLineNumbers
token, err := client.TokenIntents.Get(
		context.TODO(),
		"665bf471-d675-4165-9336-4fc9c1a73fad",
	)
```
...
| `bank`        | _object_ | The [Bank Details](/docs/api/tokens#bank-details) when `type` is `bank`, otherwise `null`.                                                                                            |
| `network_token` | _object_ | The [Card Details](/docs/api/tokens#card-details) when `type` is `network_token`, otherwise `null`.                                                                                 |
