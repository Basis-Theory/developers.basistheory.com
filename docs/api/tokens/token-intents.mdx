...
A Token Intent is a short-lived resource designed to simplify data collection in a public setting.
Token Intents serve as placeholders for sensitive data similarly to Tokens, but allow for additional validation to be
performed (e.g. Credit Card Authentication, 3DS, Fraud Checks) before being converted into a Token for long-term retention.
...
<Alert type={Alerts.WARNING}>Token Intents currently only support the `card` type.</Alert>
...
| `card`        | _object_ | The [Additional Card Details](#card-details) when `type` is `card`, otherwise `null`.                                                                                                 |
...
| Attribute          | Type     | Description                                                             |
|--------------------|----------|-------------------------------------------------------------------------|
| `bin`              | _string_ | Six to eight digit BIN of the credit card                               |
| `last4`            | _string_ | Last four digits of the credit card                                     |
...
|---------------------|---------------------------------------------------------------------------------------------|
| `sca_required`      | Indicates that Strong Customer Authentication (SCA) is required (e.g. [3DS](/docs/api/3ds)) |
