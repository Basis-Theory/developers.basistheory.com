...
Token token = new TokensClient(ClientOptions.builder().build())
    .tokenize(new HashMap<String, String>() {{
        put("first_name", "John");
Token token = client.tokens().tokenize(new HashMap<String, String>() {{
    put("first_name", "John");
    put("last_name", "Doe");
}});
```
...
```java showLineNumbers
Token token = client.tokens().tokenize(Token.builder()
    .type("token")
    .data("Sensitive Value")
...
```go showLineNumbers
result, err := client.Tokens.Tokenize(ctx, &basistheory.TokenizeRequest{
    Type: "token",
    Data: "Sensitive Value",
...
```java showLineNumbers
Token cardToken = client.tokens().tokenize(Token.builder()
    .type("card")
...
List<Object> result = client.tokens().tokenize(tokens);
```
...
```java showLineNumbers
Map<String, Object> result = client.tokens().tokenize(new HashMap<String, Object>() {{
    put("first_name", "John");
    put("last_name", "Doe");
...
result, err := client.Tokens.Tokenize(ctx, map[string]interface{}{
    "first_name": "John",
    "last_name": "Doe",
    "primary_card": &basistheory.TokenizeRequest{
        Type: "card",
        Data: map[string]interface{}{
            "number": "4242424242424242",
            "expiration_month": 12,
