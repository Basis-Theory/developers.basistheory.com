...
The [@basis-theory/basistheory-reactor-formulas-sdk-js](https://www.npmjs.com/package/@basis-theory/basis-theory-reactor-formulas-sdk-js)
npm package can be referenced within your reactor code to better control the status codes and messages included on reactor errors.
...
a standard Basis Theory [error response](/docs/api/errors) with the corresponding status code and detail specified in the table above.

If a [CustomHttpResponseError](#custom-errors) is thrown from a reactor, the status code, headers,
and body defined by the error will be used to construct a custom API response.
...
Each of the [error types](#error-types), excluding `CustomHttpResponseError`, accepts a constructor argument that will be returned within the `errors` block.
