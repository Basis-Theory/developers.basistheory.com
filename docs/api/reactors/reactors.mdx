...
const bt = await new BasisTheory().init("<Management API Key>");
...
var client = new ReactorClient("<Management API Key>");
...
public class Example {
    public static void main(String[] args) throws Exception {
        ...
        ApiKey.setApiKey("<Management API Key>");
...
with basistheory.ApiClient(configuration=basistheory.Configuration(api_key="<Management API Key>")) as api_client:
...
<Alert>
  The Patch Reactors endpoint uses a different content-type to support
...
| `configuration`  | false     | _object_ | `null`  | A key-value map of all configuration name and values for a [Reactor Formula configuration](/docs/api/reactors/reactor-formulas#reactor-formula-configuration)                                                                                                                             |
| `application.id` | false    | _uuid_   | `null`  | This Application's API key is injected into a pre-configured [BasisTheory JS SDK](/docs/sdks/server-side/node) instance passed into the Reactor's runtime. Must be a `public` or `private` [Application Type](/docs/api/applications#application-types). |
...
<Alert>
  The maximum payload size to invoke a Reactor is 5 MB.
  You could pull in more data using the injected <code>BT</code> client from within the
  Reactor code. <a href="https://basistheory.com/contact">Please reach out</a> if you need help.
</Alert>
...
const bt = await new BasisTheory().init("<Private API Key>");
...
var client = new ReactorClient("<Private API Key>");
...
public class Example {
    public static void main(String[] args) throws Exception {
        ...
        ApiKeyAuth ApiKey = (ApiKeyAuth) defaultClient.getAuthentication("ApiKey");
        ApiKey.setApiKey("<Private API Key>");
...
with basistheory.ApiClient(configuration=basistheory.Configuration(api_key="<Private API Key>")) as api_client:
    reactors_client = reactors_api.ReactorsApi(api_client)
...
| Parameter      | Required | Type     | Default | Description                                                                                                                                                                                                       |
| -------------- | -------- | -------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `args`         | false    | _object_ | `null`  | Arguments to provide to the reactor. These are required when arguments have been defined as [request parameters](/docs/api/reactors/reactor-formulas#reactor-formula-request-parameters) on a Reactor Formula.    |
...
When invoking a reactor, you may provide any `args` within your request without first pre-declaring `request_parameters` within the [Reactor Formula](/docs/api/reactors/reactor-formulas#reactor-formula-object).
However, if the formula declares any `request_parameters`, then matching `args` will be validated against this contract.
Argument validation occurs when a property within `args` exactly matches the name of a request parameter - this comparison is case-sensitive.
Additional arguments included within `args` that are not declared as request parameters will not be validated and will be passed into the reactor.

Nested objects may be declared as request parameters by dot-separating levels of the object hierarchy.
...
All required `request_parameters` (those with `optional: false`) must be included within the `args` on each request or the request will be rejected with a 400 error.
If a request parameter is declared as optional, then a value for this parameter may be omitted within `args`.

Each validated argument must match the `type` on the corresponding request parameter, and if the type does not match, auto casting to the declared type will be attempted.
...
If an expression is provided within the `args` for a declared request parameter, then auto type casting is performed after detokenization.
