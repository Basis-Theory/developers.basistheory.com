...
## Invoke a Reactor Asynchronously <Enterprise />
...
## Retrieve Result <Enterprise />

Will retrieve the results of an asynchronous invocation.

<HttpMethod
  method={HttpMethods.GET}
  endpoint="https://api.basistheory.com/reactors/{id}/requests/{request_id}"
/>
...
<Tabs className="bt-tabs" groupId="languages">
  <TabItem value="shell" label="cURL">
```shell showLineNumbers
curl --location 'https://api.basistheory.com/reactors/7001A144-96D0-4401-9D96-13557824A9E0/requests/369a879e-338d-4f52-83cf-53a6af3f3f18' \
...
| --------- | -------- | ------ | ------- | --------------------- |
| `id`      | true     | _uuid_ | `null`  | The ID of the Reactor |
| `request_id`      | true     | _uuid_ | `null`  | The ID of the Reactor request id. Retrieved from either the response of `react-async` or the `reactor.completed-async` [webhook event](/docs/api/webhooks/eventdata#reactor-completed-asynchronously) |
