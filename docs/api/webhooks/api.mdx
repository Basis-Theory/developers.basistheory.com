...
Webhook webhook = client.webhooks().create(Webhook.builder()
    .name("webhook-create")
    .url("https://www.example.com")
...
```
  ...
  <TabItem value="go" label="Go">
    ```go showLineNumbers
    webhooks, err := client.Webhooks.List(ctx, &basistheory.WebhooksListRequest{})
    ```
  </TabItem>
</Tabs>
...
```java showLineNumbers
Webhook webhook = client.webhooks().get("id");
```
...
```java showLineNumbers
Webhook updatedWebhook = client.webhooks().update("id", Webhook.builder()
    .name("webhook-update")
    .url("https://www.example1.com")
    .notifyEmail("notify@basistheory.com")
    .status("enabled")
...
```
    ...
    <TabItem value="java" label="Java">
```java showLineNumbers
List<String> eventTypes = client.webhooks().events().list();
```
  </TabItem>
  <TabItem value="go" label="Go">
    ```go showLineNumbers
