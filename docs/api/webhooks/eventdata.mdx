...
```json title="3ds.session.method-completed"
{
  "session": {
    "id": "...",
    "type": "...",
...
<Alert>This event just indicates that the challenge was completed, not that it succeeded or failed. To see the result of the challenge, perform a <a href="/docs/api/3ds/sessions#get-challenge-result">Get Challenge Result</a> API call.</Alert>

```json title="3ds.session.challenge-completed"
{
  "session": {
    "id": "...",
    "type": "...",
...
```json title="3ds.session.decoupled-challenge-notification"
{
  "session": {
    "id": "...",
    "type": "...",
...
[Reactor Errors](/docs/api/proxies/invoke-pre-configured-proxies#proxy-transform-errors) and [Custom Errors](/docs/api/reactors/invoke-reactors#reactor-error-codes)

```json title="Standard proxy error"
"error": {
  "errors": {
      "error": [
...
| request.id              | String | (Optional) The unique identifier of the request if the `invocation_type` is `async`. Can be used to [retrieve the results](/docs/api/reactors#retrieve-result) of the invocation. |
...
```json title="token-intent.created"
{
  "token_intent": {
    "id": "...",
    "type": "...",
...
| Name                     | Type   | Description                                                                                      |
|--------------------------|--------|--------------------------------------------------------------------------------------------------|
| token_intent.id          | String | The unique identifier of the [token intent](/docs/api/tokens/token-intents#token-intent-object). |
...
```json title="token-intent.deleted"
{
  "token_intent": {
    "id": "..."
  }
...
```json title="token-intent.converted"
{
  "token_intent": {
    "id": "...",
    "type": "..."
...
As mentioned in the [Google Pay™ API documentation](/docs/api/google-pay/api#create) either a Google Pay™ resource or a Basis Theory's [Token Intent](/docs/api/tokens/token-intents#token-intent-object) can be created based on the provided encrypted token. Please refer to [Token Intent events documentation](/docs/api/webhooks/eventdata#token-intents) for information about token intents created through the Google Pay™ API.

### Resource Created

```json title="google-pay.created"
{
  "google_pay": {
    "id": "...",
    "status": "...",
...
```json title="google-pay.retrieved"
{
  "google_pay": {
    "id": "..."
  }
