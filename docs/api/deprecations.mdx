...
| `bt` parameter in Reactors and Proxy Transforms                    | October 13, 2022  | TBD  | The `bt` parameter in Reactors and Proxy Transforms has been deprecated and replaced by the `applicationOptions` parameter + [Basis Theory SDK](/docs/sdks/server-side/node). For more information, see the section below on [Migrating from Reactors and Proxy Transforms](#migrating-from-reactors-and-proxy-transforms). |

## Migrating from `bt` parameter in Reactors and Proxy Transforms

We have replaced the helper `bt` parmater being passed in with a more traditional method by providing our SDK as a dependency. To migrate your existing code, you will need to update your code to use the SDK and the new `applicationOptions` parameter which contains configuration options for your selected Application:

1. Require the SDK in your code 
2. Create a new BasisTheoryClient with the `applicationOptions` parameter.
