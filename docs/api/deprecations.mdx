...
2. [Invoke a Reactor Asynchronous](/docs/api/reactors/#invoke-a-reactor-async) with the new `/reactors/:id/react-async` endpoint.
3. Retrive the [Async Reactor Results](/docs/api/reactors#retrieve-result)
    1. This ensures the system receiving the webhook has access to the results of the reactor.
4. Process them in the same way you are currently doing with the `callback_url`
