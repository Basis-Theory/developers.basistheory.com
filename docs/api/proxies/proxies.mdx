...
<Alert>
  If the destination endpoint takes more than 25 seconds to respond, a <code>408 Request Timeout</code> error will be returned.
</Alert>
...
<Alert>
  If the <code>BT-DESTINATION-STATUS</code> header is not present in the response, there is likely an integration or service problem, with 4xx codes indicating issues on your end and 5xx codes indicating problems with Basis Theory's service.
  <br />
  <br />
  However, if the <code>BT-DESTINATION-STATUS</code> HTTP header is present and has an HTTP status code, the code represents the status code responded from the destination service where a 4xx code generally points to a problem with the integration to the destination and a 5xx code suggests an issue with the destination service itself.
</Alert>
...
<Alert>
  We can also support single dedicated outbound IP addresses. Please <a href="https://basistheory.com/contact">reach out</a> to our support team if you have specific security requirements that mandate this configuration.
</Alert>
...
<Alert>
  For more detailed examples about how to detokenize within the Proxy, check out our <a href="/docs/expressions/detokenization#examples">Detokenization Examples</a>.
</Alert>
