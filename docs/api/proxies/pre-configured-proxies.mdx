...
| `basisTheoryConfig`| _object_ | An object capable of being used when initializing the [Basis Theory JS SDK](/docs/sdks/server-side/node). Example: `const client = new BasisTheoryClient(basisTheoryConfig);` |
...
| `configuration` | _object_ | The configuration defined for the [Proxy object](#proxies-proxy-object)                                                   |
| `bt`            | _object_ | A pre-configured Basis Theory JS instance if an `application` was configured on the [Proxy object](#proxies-proxy-object) |
| `basisTheoryConfig`| An object capable of being used when initializing the [Basis Theory JS SDK](/docs/sdks/server-side/node). Example: `const client = new BasisTheoryClient(basisTheoryConfig);` |
