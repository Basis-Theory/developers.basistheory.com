...
var client = new ProxyClient("<Management API Key>");
...
<Alert>
  Transforms are only currently supported for requests with content type{" "}
  <code>application/json</code> or{" "}
  <code>application/x-www-form-urlencoded</code>. Please{" "}
  <a href="https://basistheory.com/contact">let us know</a> if you would like to
  use proxy transforms with other content types.
</Alert>

<Alert>
  The maximum payload size to invoke a Pre-configured proxy which uses a
  Transform is 5 MB. You could pull in more data using the injected{" "}
  <code>BT</code> client from within the Transform code.{" "}
  <a href="https://basistheory.com/contact">Please reach out</a> if you need
  help.
</Alert>
