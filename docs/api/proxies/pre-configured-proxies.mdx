...
| `configuration`      | false    | _map\<string, string>_                                                      | `null`  | A string key and string value map of all configuration name and values for a [Proxy Transform](#proxy-transform-object)                                                                                                                                            |
...
| `request_transform`  | _[Request Proxy Transform](#request-proxy-transform-object)_ | (Optional) [Request Transform](#request-transforms) which will be executed against the proxy request before sending to the destination.                                                                                                                     |
| `response_transform` | _[Response Proxy Transform](#response-proxy-transform-object)_ | (Optional) [Response Transform](#response-transforms) which will be executed against the destination's response before returning the proxy response.                                                                                                        |
...
| `configuration`      | _map\<string, string>_                                        | (Optional) A string key and string value map of all configuration name and values for a [Proxy Transform](#proxy-transform-object)                                                                                                                                            |
...
### Request Proxy Transform Object
...
<Alert>The maximum payload size to invoke a Pre-configured proxy which uses a Transform is 5 MB. You could pull in more data using the injected <code>BT</code> client from within the Transform code. <a href="https://basistheory.com/contact">Please reach out</a> if you need help.</Alert>
...
Mask response transforms can only be used to transform the body of the response received from the destination before returning a response from the proxy.
...
In the example above, the value of the `method` json attribute will be replaced with `*`.
If the method were `GET`, the replacement would be `***`. 
If the method were `POST`, then `****` would be the replacmenet.
...
Given an input string of `aabbccaabbccaa`, the entire string will processed during masking. 
*ALL* values that match the capture group value, in this case `aa`, will be replaced.
The string will then be masked as `**bbcc**bbcc**`.

##### Chase Stratus PAN Matcher

If your proxy is interfacing with the Chase Stratus service, you may be able to take advantage of a pre-configured mask.
