...
Pre-Configured Proxies differ from the [Ephemeral Proxy](/docs/api/proxies/ephemeral-proxy) by requiring setup beforehand. You define a proxy instance with a unique URL and fixed settings, which can be reused across requests or shared with third parties for inbound HTTP requests (e.g., providing an integrator with a proxy URL). This approach suits scenarios needing consistent configurations or stricter controls, such as whitelisting destinations in production environments.

Additional options for Pre-Configured Proxies include:

- **Request and response transforms**: Optionally use custom Node.js code to alter requests or responses—useful for specific cases, but not mandatory. You can also use specialized transform types like `tokenize` (for creating tokens from encrypted data) and `append_json` (for injecting values into JSON responses) which provide alternatives to custom code.
...
| `request_transforms` | false    | _array\<[Proxy Request Transform](#proxy-request-transform-object)>_   | `null`  | Array of [Request Transforms](#request-transforms) which will be executed sequentially against the proxy request before sending to the destination.                                                                                             |
| `response_transforms`| false    | _array\<[Proxy Response Transform](#proxy-response-transform-object)>_ | `null`  | Array of [Response Transforms](#response-transforms) which will be executed sequentially against the destination's response before returning the proxy response.                                                                               |
...
| `request_transforms` | _array\<[Proxy Request Transform](#proxy-request-transform-object)>_   | (Optional) Array of [Request Transforms](#request-transforms) which will be executed sequentially against the proxy request before sending to the destination.                                                                                      |
| `response_transforms`| _array\<[Proxy Response Transform](#proxy-response-transform-object)>_ | (Optional) Array of [Response Transforms](#response-transforms) which will be executed sequentially against the destination's response before returning the proxy response.                                                                         | 
...
| Attribute | Type     | Description                                                                                                                             |
|-----------|----------|-----------------------------------------------------------------------------------------------------------------------------------------|
| `type`    | _string_ | The type of request transform. One of `code` or `tokenize`. Default: `code`.                                                           |
| `code`    | _string_ | Transform code which will be executed when the Proxy transform is processed. See [Request Transforms](#request-transforms) for details. *Required* when `type` is `code` |
| `options` | _object_ | Transform options object. *Required* when `type` is `tokenize`. See [Tokenize Transform Options](#tokenize-transform-options) for details. |

#### Tokenize Transform Options

| Attribute    | Type     | Description                                                                                                                                           |
|--------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| `token`      | _object_ | A [Create Token Request](/docs/api/tokens#create-token) object defining the token to be created. Supports Liquid expressions with the `encrypted` keyword when using a `BT-ENCRYPTED` header. *Required* |
| `identifier` | _string_ | A unique identifier for this token that can be referenced in request body expressions using `tokenize_transform` or response transforms using `transform_identifier`. Must contain only alphanumeric characters, hyphens, and underscores. Maximum length of 100 characters. See [Advanced Transform Types](#advanced-transform-types) for details. |
...
| Attribute     | Type     | Description                                                                                                                                                                          |
|---------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `type`        | _string_ | The type of response transform. One of `code`, `mask`, or `append_json`. Default: `code`.                                                                                           |
...
| `options`     | _object_ | Transform options object. *Required* when `type` is `append_json`. See [Append JSON Transform Options](#append-json-transform-options) for details.                                |

#### Append JSON Transform Options

| Attribute  | Type     | Description                                                                                                                                                                                    |
|------------|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `value`    | _string_ | The value to append to the JSON response. Supports Liquid expressions including `transform_identifier` to reference tokens created in request transforms. *Required*                        |
| `location` | _string_ | JSONPath expression specifying where to add the value in the response JSON. For example, `$.created_token_id` adds the value as a top-level property named `created_token_id`. *Required* |
...
Beyond the traditional code-based transforms, Pre-configured Proxies support specialized transform types for common operations:

### Tokenize Request Transforms

Tokenize request transforms allow you to create tokens from data before sending the request to your destination. This is useful for:

- Converting sensitive data from encrypted payloads into tokens
- Creating tokens with mixed static and dynamic properties
- Generating referenceable tokens that can be used in response transforms

#### Basic Tokenize Transform Example

```json showLineNumbers
{
  "name": "My Tokenizing Proxy",
...
#### Using Tokenized Data in Request Body

Once tokens are created via request transforms, you can reference them in your request body using the `tokenize_transform` expression:

```json showLineNumbers
{
  "card_token_id": "{{ tokenize_transform: 'payment_card' | json: '$.id' }}",
  "card_brand": "{{ tokenize_transform: 'payment_card' | json: '$.card.brand' }}",
  "customer_info": {
    "holder_name": "{{ tokenize_transform: 'payment_card' | json: '$.metadata.card_holder' }}"
  }
}
```

### Append JSON Response Transforms

Append JSON response transforms allow you to modify the JSON response from your destination by adding new properties. This is useful for:

- Injecting token IDs into responses for client reference
- Adding metadata about the tokenization process

#### Basic Append JSON Transform Example

```json showLineNumbers
{
  "name": "My Response-Enhancing Proxy",
  "destination_url": "https://api.example.com/payments",
  "request_transforms": [
    {
      "type": "tokenize",
  ...
  "response_transforms": [
    {
      "type": "append_json",
      "options": {
        "value": "{{ transform_identifier: 'created_card_token' | json: '$.id' }}",
...
When using tokenize transforms with the `BT-ENCRYPTED` header, the proxy can merge encrypted data with static token properties defined in your transform configuration.

### Encrypted Data Merge Behavior

For outbound proxies with a `BT-ENCRYPTED` header, the proxy will:

1. **Merge Properties**: Combine properties from the `BT-ENCRYPTED` header with the `options.token` block
2. **Static + Dynamic**: This allows for partially static and dynamic properties in token creation

#### Example: Encrypted Data with Static Metadata
  ...
  "request_transforms": [
    {
      ...
      "options": {
        "token": {
          "type": "card",
          "data": "fallback-data-if-no-encrypted",
          "metadata": {
            "source": "proxy_transform",
            "processing_date": "2024-01-15"
          }
        },
        "identifier": "merged_card_token"
      }
    }
  ]
}
...
**BT-ENCRYPTED Header Content:**
```json showLineNumbers
{
  "type": "card",
  "encrypted": "<JWE_ENCRYPTED_CARD_DATA>",
  "metadata": {
...
**Resulting Token Creation:**
```json showLineNumbers
{
  "type": "card",
  "data": "<DECRYPTED_CARD_DATA>", // Replaced from encrypted
  "metadata": {
    "source": "proxy_transform",      // From transform config
    "processing_date": "2024-01-15", // From transform config  
    "customer_id": "cust_12345"       // From BT-ENCRYPTED header
  }
}
```
...
When creating tokens using `proxy:invoke` or `token:use` permissions:

- **Token data is redacted** - The actual token data will not be revealed in responses
- **Token IDs are accessible** - You can safely reference token IDs in expressions

This ensures that sensitive data remains protected while still allowing you to build functional proxy workflows.
