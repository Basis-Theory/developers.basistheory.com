...
Proxy proxy = client.proxies().create(Proxy.builder()
    .name("My Proxy")
    .destinationUrl("https://example.com/api")
    .requestTransform(new ProxyTransform("module.exports = async function (req) { return { headers: req.args.headers, body: req.args.body }; };"))
    .responseTransform(new ProxyTransform("module.exports = async function (req) { return { headers: req.args.headers, body: req.args.body }; };"))
...
```java showLineNumbers
Page<Proxy> page = client.proxies().list();
```
...
```java showLineNumbers
Proxy proxy = client.proxies().get("id");
```
...
```java showLineNumbers
Proxy updatedProxy = client.proxies().update("id", Proxy.builder()
    .name("My Proxy")
    .destinationUrl("https://example.com/api")
    .requestTransform(new ProxyTransform("module.exports = async function (req) { return { headers: req.args.headers, body: req.args.body }; };"))
    .responseTransform(new ProxyTransform("module.exports = async function (req) { return { headers: req.args.headers, body: req.args.body }; };"))
...
```go showLineNumbers
proxy, err := client.Proxies.Patch(ctx, "id", &basistheory.PatchProxyRequest{
    Name: "My Proxy",
    DestinationURL: "https://example.com/api",
    Configuration: map[string]string{
        "SERVICE_API_KEY": "key_abcd1234",
...
```java showLineNumbers
Proxy patchedProxy = client.proxies().patch("id", Proxy.builder()
    .name("My Proxy")
    .destinationUrl("https://example.com/api")
    .configuration(new HashMap<>())
    .build());
