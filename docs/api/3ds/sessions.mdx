...
| `type`            | true     | _string_ | The 3DS session type. Defaults to `customer`.  Use `merchant` for merchant initiated transactions (MIT). |
...
| `sdk_ephemeral_public_key`   | _object_                                         | Public key component of the ephemeral key pair generated by the SDK (`mobile` device only). |
