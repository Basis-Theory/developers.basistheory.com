...
const session = await bt.threeds.get("c06d0789-0a38-40be-b7cc-c28a718f76f1");
```
...
var session = await client.GetByIdAsync("c06d0789-0a38-40be-b7cc-c28a718f76f1");
```
...
public class Example {
    public static void main(String[] args) throws Exception {
        ...
        ThreeDsSession result = apiInstance.getById("c06d0789-0a38-40be-b7cc-c28a718f76f1");
    }
}
```
...
```json showLineNumbers
{
    ...
    "method": {
        "method_url": "https://acs.cardissuer.com/3ds/acs/browser/method",
...
const token = await bt.threeds.authenticate("c06d0789-0a38-40be-b7cc-c28a718f76f1", {
...
var result = await client.GetChallengeResult("c06d0789-0a38-40be-b7cc-c28a718f76f1");
```
...
| `pan_token_id` | _string_ | The ID for the card token used in the 3DS transaction |
...
| `method_completion_indicator` | _string_ | Indicator of if the method request completed successfully or not |
...
| `authentication_status` | _string_ | The outcome of the 3DS authentication |
| `authentication_status_reason` | _string_  | Additional information about the authentication status if necessary |
| `eci` | _string_ | Electronic Commerce Indicator  |
| `acs_challenge_mandated` | _string_ | Indicates whether regional mandates (e.g. PSD2) required a challenge to be performed |
| `authentication_challenge_type` | _string_ | The type of challenge authentication used (if challenge) |
...
| `challenge_cancel_reason` | _string_ | The reason why a challenge was cancelled |
| `cardholder_info` | _string_ | Information from the issuer to be displayed to the cardholder |
| `whitelist_status` | _string_ | Indicates if the cardholder whitelisted the merchant |
| `whitelist_status_source` | _string_ | Identifies the system that set the whitelist value |
