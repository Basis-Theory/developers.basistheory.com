...
ThreeDSSession session = new ThreedsClient(ClientOptions.builder().build())
    .sessions()
    .create(ThreeDSSession.builder()
        .tokenId("af9f65fd-932f-4b3d-960a-1bd7a1d14d18")
        .type("merchant")
        .build());
```
  ...
  <TabItem value="go" label="Go">
    ```go showLineNumbers
    createResponse, createErr := client.Threeds.Sessions.Create(ctx, &threeds.CreateThreeDsSessionRequest{
      TokenId: "af9f65fd-932f-4b3d-960a-1bd7a1d14d18",
      Type:    "merchant",
    })
    ```
...
| `authentication_request` | conditional       | _object_ | Authentication request details for redirect flows. See [Authentication Request](#authentication-request-object) for details. Required when using redirect implementation. |
| `callback_urls`          | conditional       | _object_ | Success and failure callback URLs for redirect flows. See [Callback URLs](#callback-urls-object) for details. Required when using redirect implementation.                |
...
<Alert type={Alerts.INFO}>
  <strong>Redirect Implementation:</strong> If you include both <code>authentication_request</code> and <code>callback_urls</code> parameters when creating a customer session, the response will include a <code>redirect_url</code> that you can use to redirect customers to the Basis Theory 3DS authentication page. This approach allows you to create CIT sessions from your backend without using frontend SDKs. See the <a href="/docs/guides/threeds/implementation-cit-redirect">3DS CIT with Redirection guide</a> for more details.
</Alert>
...
| `card_brand`                   | false       | _string_                                   | The chosen card brand to route the transaction through (for co-badged cards).                                                                                                    |
...
| Attribute | Required | Type     | Description                                                    |
|-----------|----------|----------|----------------------------------------------------------------|
| `success` | true     | _string_ | The URL to redirect customers to after successful authentication. |
| `failure` | true     | _string_ | The URL to redirect customers to after failed authentication.     |
...
```json showLineNumbers
{
  ...
  "redirect_url": "https://api.basistheory.com/3ds/redirect/c0c22fcd-d42c-497e-a9a6-2eacd31770d7?apiKey=<PUBLIC_API_KEY>"
}
```
...
| `cardBrand`               | _string_ | The main brand for the card used in the 3DS transaction.                                                   |
| `additional_card_brands`  | _array_  | Array of all brands the card was identified as (if co-badged).                                             |
...
| `redirect_url`            | _string_ | The URL to redirect customers to for 3DS authentication (only present when using redirect implementation). |
...
ThreeDSSession session = new ThreedsClient(ClientOptions.builder().build())
    .sessions()
    .get("c0c22fcd-d42c-497e-a9a6-2eacd31770d7");
```
  ...
  <TabItem value="go" label="Go">
    ```go showLineNumbers
    getResponse, getErr := client.Threeds.Sessions.Get(ctx, sessionId)
    ```
  </TabItem>
</Tabs>
...
```java showLineNumbers
ThreeDsAuthentication session = new ThreedsClient(ClientOptions.builder().build())
    .sessions()
        .authenticate("c0c22fcd-d42c-497e-a9a6-2eacd31770d7", AuthenticateThreeDsSessionRequest.builder()
            .authenticationCategory("payment")
            .authenticationType("payment-transaction")
            .challengePreference("no-challenge")
            .merchantInfo(ThreeDsMerchantInfo.builder()
            .mid("9876543210001")
            .acquirerBin("000000999")
            .name("Example 3DS Merchant")
            .categoryCode("7922")
    ...
    .requestorInfo(ThreeDsRequestorInfo.builder()
        .amexRequestorType("MER")
        .cbSiretNumber("78467169500087")
        .build()
    )
    .purchaseInfo(ThreeDsPurchaseInfo.builder()
        .amount("80000")
        .currency("826")
        .exponent("2")
        .date("20240109141010")
    ...
    .cardholderInfo(ThreeDsCardholderInfo.builder()
        .name("John Doe")
        .email("john@example.com")
        .build()
    )
...
```
  ...
  <TabItem value="go" label="Go">
    ...
    authenticateResponse, authenticateErr := client.Threeds.Sessions.Authenticate(ctx, sessionId, authRequest)
    ```
  </TabItem>
</Tabs>
...
ThreeDsAuthentication session = new ThreedsClient(ClientOptions.builder().build())
    .sessions()
    .getChallengeResult("c0c22fcd-d42c-497e-a9a6-2eacd31770d7");
```
...
| `authentication_status_reason_code` | _string_ | EMV numeric code for the authentication status reason.                                                                                       |
...
Errors generated by the Basis Theory API (e.g., invalid API keys or validation errors) follow the standard [API Error](/docs/api/errors) formatting.

However, errors that occur externally during the 3DS flow (e.g., errors originating from the ACS, Directory Server, or 3DS Server) will return a specific `424` [3DS Service Error](#3ds-service-error).

<Alert type={Alerts.INFO}>For <code>424</code> errors, it is recommended to fully retry the transaction by creating a new session if the inner <code>service_status</code> is <code>502</code> or <code>504</code>.</Alert>


### 3DS Service Error

**3DS Service Errors** are returned when an error occurs within the 3DS Server, Directory Server (DS), or Access Control Server (ACS) during authentication.

The API responds with a **424** HTTP status code and the following error format:
...
| `status`            | _integer_                                        | HTTP status code of the response. Always (424) for these errors.            |
| `error`             | [3DS Service Error](#3ds-service-error-object)   | Error object with specific details about the returned 3DS Service Error     |

#### 3DS Service Error Object

| Attribute        | Type     | Description                          |
|------------------|----------|--------------------------------------|
| `service_status` | _string_ | The status code returned by the service. |
| `session_id`     | _string_ | The ID of the affected 3DS session.          |
| `error_source`   | _string_ | The source of the error (3DS Server, Access Control Server, Directory Server). |
| `message`        | _string_ | Short description provided by the external 3DS service.                        |
| `detail`         | _string_ | Additional detailed information regarding the error.                           |
