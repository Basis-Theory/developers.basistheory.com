...
<Alert type={Alerts.INFO}>Webhooks are coming soon. Contact <a href="mailto:support@basistheory.com">support@basistheory.com</a> to request notification when they are publically available.</Alert>

Configure a webhook to retrieve notifications for job status changes. You are able to subscribe to webhooks for events of type `account-updater.job.created`, `account-updater.job.completed`, or `account-updater.job.failed`.
