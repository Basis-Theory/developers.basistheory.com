...
<Alert>Don't want to complete this guide? View the completed example project <a href="https://github.com/Basis-Theory-Labs/inbound-detokenization-example">here</a>.</Alert>

## Creating the Inbound Proxy

In order to listen to API requests, we will create a Proxy that sits between the Client and your Server. We will assume these requests don't contain any sensitive information, so we will pass them through to the Server, which will respond with tokenized data. The Proxy will then [transform](/docs/api/proxies/pre-configured-proxies#transforms) the Server response to detokenize it and respond to the Client with the plaintext data.
...
<Alert>Save the API Key from the created Management Application as it will be used later in the next step.</Alert>
...
<Alert>Save the API Key from the created Public Application as it will be used in the next step.</Alert>
...
<Alert>In a real world scenario, the data used to query the User Data may be different, such as the User id stored in the Server database.</Alert>
