...
More advanced Proxy scenarios can be configured via the [Proxies endpoint](/docs/api/proxies/pre-configured-proxies), such as tokenizing inbound cardholder data before it touches your API or encrypting outbound API calls with an encryption key.
