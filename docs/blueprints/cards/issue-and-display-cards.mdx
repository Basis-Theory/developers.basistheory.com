...
class ViewController: UIViewController {
    ...
    @IBAction func reveal(_ sender: Any) {
        // highlight-start
        BasisTheoryElements.createSession(apiKey: "<API Key>") { data, error in
...
<Alert>
  Be sure to replace <code>&lt;API Key></code> with the Public API Key you
...
<Alert>
  Be sure to replace <code>&lt;Private API Key></code> with the Private API Key you
...
app.post("/authorize", async (request, response) => {
  ...
  app.post("/create-proxy", async (request, response) => {
    const proxy = await bt.proxies.create(
      ...
      { apiKey: "<Management API Key>" } // management application
    );
...
<Alert>
  Be sure to replace <code>&lt;Management API Key></code> with the Management API Key you
