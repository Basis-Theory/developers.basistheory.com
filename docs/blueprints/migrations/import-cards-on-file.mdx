...
```text
from: John Doe <psp_account_owner@mycompany.com>
to: PSP Support <support@mypsp.com>
cc: Basis Theory Support <support@basistheory.com>
subject: Request for Token Export

Dear { PSP } team,

I would like to proceed with a { full / partial } card on file / tokens export.
My new provider is:
...
1. Timeline - we plan to complete the migration process by { insert target date }. Any guidance or support to expedite this trans
2. Data format - CSV

```


1. Format - specify that the
typically, payment processors provide the exported data in common formats, such as CSV or JSON. If you are given the choice, Basis Theory prefers CSV files.
2. Transmission - most common method of securely transmitting export files are via SFTP, but some processors may utilize SCP or FTP over SSL.
3. Volume - partial or full migrations may
...
Your Basis Theory Account Manager can provide you with the latest AOC.
