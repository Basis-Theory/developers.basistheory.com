...
This blueprint explores how to perform API-based migration of Personal Identifiable Information (PII) from an existing database into Basis Theory platform.
...
- [PostgreSQL](https://www.postgresql.org/) for the relational database, which is assumed to be already setup;
- [Prisma](https://www.prisma.io/) for the ORM layer;
- TypeScript programming language for the migration script;

<Alert>
  Already have an existing database? Skip to <a href="#migrate-data">Migrate Data</a> section.
</Alert>
...
In this section, we will import existing sensitive data from our database into Basis Theory platform by calling the [Tokenize API](/docs/api/tokens/tokenize). The elected data points are `ssn`, `email` and `phoneNumber`, which after being migrated, will be replaced in the database with synthetic equivalents (aka [Token ID](/docs/api/tokens/#token-object)) by employing different [aliasing](/docs/concepts/what-are-tokens#aliasing) techniques. Additionally, we will also customize the [masking](/docs/concepts/what-are-tokens#masking) of the data points for securely revealing the data partially to our systems. For example:

| Data type     | Value                 | Token Id              | Masked Data        |
|---------------|-----------------------|-----------------------|--------------------|
...
And finally, let's iterate through the data chunks and create a token per each sensitive data field we want to tokenize. In this scenario, we are going to secure `ssn`, `email` and `phoneNumber`.
      ...
      const tokens = await bt.tokenize(
        chunk.map((p) => ({
          ...
          email: {
            ...
            data: p.email,
            mask: '{{ data | slice: 0 }}####@{{ data | split: "@" | last }}',
            containers: ["/pii/high/"],
            privacy: {
              classification: "pii",
              impact_level: "high",
