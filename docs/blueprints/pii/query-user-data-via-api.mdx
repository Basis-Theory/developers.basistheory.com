...
import SequenceDiagram from "@site/static/img/getting-started/blueprints/pii/blueprint-query-user-data-via-api-sequence-diagram.svg";

# Query User Data via API
...
Add the Private Application resource to your Terraform configuration:
...
```bash showLineNumbers
curl --location --request POST 'https://api.basistheory.com/tokens' \
--header 'BT-API-KEY: test_1234567890' \
