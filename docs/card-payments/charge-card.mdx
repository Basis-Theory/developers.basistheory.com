...
Once you've securely saved a card in the vault and generated a persistent token, you are ready to perform subsequent charges. Whether you have returning customers purchasing with a saved card, or you are performing

Talk about these:
- Returning customers
  - Recollect CVC (PSP requirement or risk appetite)
- Managed subscription charges

### Using the PAN

- why? go to a processor different from the one used in the first charge (at least the first time)

### Using the PSP Token

- why? not go through BT,
- (automatic) processor payload optimization
  - auto NTs, RTAU, NT TX ID
