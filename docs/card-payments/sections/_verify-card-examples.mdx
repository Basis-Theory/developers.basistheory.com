...
-H 'X-API-Key: Bearer <CHECKOUT_SECRET_KEY>' \
// highlight-end
...
```
  ...
  In Ayden, you can verify a card without charging it [by calling the `/payments` API with 0 value amount.](https://docs.adyen.com/api-explorer/Checkout/latest/post/payments).
  ```shell showLineNumbers title="Verify a Card"
curl 'https://api.basistheory.com/proxy' \
-X 'POST' \
// highlight-start
...
-H 'X-API-Key: YOUR_AYDEN_API_KEY' \
// highlight-end
-H 'Content-Type: application/json' \
--data '{
  {
  ...
  "paymentMethod": {
    "type": "scheme",
    // highlight-start
    "encryptedCardNumber": "{{ token_intent: d2cbc1b4-5c3a-45a3-9ee2-392a1c475ab4 | json: "$.data.number" }}",
    "encryptedExpiryMonth": "{{ token_intent: d2cbc1b4-5c3a-45a3-9ee2-392a1c475ab4 | json: \"$.data\" | card_exp: \"MM\" }}",
    "encryptedExpiryYear": "{{ token_intent: d2cbc1b4-5c3a-45a3-9ee2-392a1c475ab4 | json: \"$.data\" | card_exp: \"YYYY\" }}",
    "encryptedSecurityCode": "{{ token_intent: d96b9925-85d7-495e-88fe-a9c3ecb23b61 | json: \"$.data.cvc\" }}"
    // highlight-end
  },
  "returnUrl": "https://your-company.com/...",
  "merchantAccount": "YOUR_AYDEN_MERCHANT_ACCOUNT"
}
}'
```
```json showLineNumbers title="Verification Response"
