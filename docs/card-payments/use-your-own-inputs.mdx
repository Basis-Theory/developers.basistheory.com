...
```mermaid
  sequenceDiagram
    ...
    frontend -->> customer: Render checkout with Card Inputs
    customer ->> frontend: Enter card data
    customer ->> frontend: Submit
    frontend ->>+ bapi: Create Token Intent
    bapi -->>- frontend:  Token intent<br>w/ card information
