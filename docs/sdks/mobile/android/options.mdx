...
The color of the icon can be changed from the default black through the `copyIconColor` attribute. 
That attribute takes in a `int` value. For example, to change the color to red, you can do the following:

```kotlin showLineNumbers
element.copyIconColor = Color.RED
```

The `android.graphics` package has a number of color constants that can be used, but you can use any custom colors by [encoding](https://developer.android.com/reference/android/graphics/Color) them as integers.
