...
Additionally, the card related event [details](#elementeventdetail-typee) can be accessed from the `cardMetadata` property only on [`CardNumberUITextField`](/docs/sdks/mobile/ios/types#cardnumberuitextfield).
