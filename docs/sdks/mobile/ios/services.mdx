...
| Error | Description |
| --- |--- |
| TokenizingError.applicationTypeNotPublic | The [Application](/docs/api/applications) API key used is not of type `public` |
| TokenizingError.invalidInput | An element instance used in a tokenize request is invalid. Check the element events on each element to determine which one is invalid |
...
<Alert>
  The <code>ErrorResponse</code> enum can be from the BasisTheory Swift SDK through <code>import BasisTheory</code>.
</Alert>
