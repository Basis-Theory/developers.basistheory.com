...
- `elementValueReference` - This property is used to get the sensitive value of a `JSON` property. It returns a value reference that is used to set the value of an iOS Basis Theory element.
...
As of now, only numbers, booleans, and strings can be retrieved using this method.
