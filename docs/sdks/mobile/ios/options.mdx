...
A [`TextElementUITextField`](/docs/sdks/mobile/ios/types#textelementuitextfield) can restrict and fill user input by using the mask attribute.
...
A [`TextElementUITextField`](/docs/sdks/mobile/ios/types#textelementuitextfield) allows you to validate user input before tokenization through the `validation` attribute of type `NSRegularExpression`.
...
<Alert>
  Validation is performed on the transformed value, if <a href="/docs/sdks/mobile/ios/options#transform">transform</a> is set. If I had not set a transform above to remove the parentheses and dashes, the validation would be performed on a
