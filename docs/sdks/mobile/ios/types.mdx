...
<Alert>
  Note that when these card-related elements are grouped together in an array,
  as shown in the value of <code>data</code> below, the token <code>type</code>{" "}
  of <code>card</code> should be used.
</Alert>
