...
Basis Theory React Native uses camel case for most methods and converts these properties to snake case when sending requests to the API. One notable exception to this is the [Tokenize](/docs/api/tokens/tokenize) method which uses snake case for the request body.
