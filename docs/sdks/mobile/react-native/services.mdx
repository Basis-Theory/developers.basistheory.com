...
const MyForm = () => {
  ...
  const getCardNumber = async () => {
    const token = await BasisTheoryElements.getTokenById('<TOKEN ID FOR CUSTOMER>', '<SESSION_API_KEY>');
...
<Alert type={Alerts.WARNING}>The <code>data</code> attribute in the token returned by the <code>getTokenById</code> method is not the actual data, but a synthetic representation of the sensitive detokenized data.</Alert>
