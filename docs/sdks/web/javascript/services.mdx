...
const textElement = BasisTheory.createElement("cardExpirationDate", {
  targetId: "expirationDate",
  value: "04/25",
});

const textElementReadOnly = BasisTheory.createElement("cardExpirationDate", {
  targetId: "expirationDateReadOnly",
  readOnly: true
});

textElementReadOnly.setValueRef(textElement) 
```

`textElementReadOnly` will be updated every time the value of `textElement` changes.
