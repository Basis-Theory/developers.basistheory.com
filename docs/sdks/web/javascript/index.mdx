...
<Alert type={Alerts.WARNING}>
  The Basis Theory JS SDK `>1.77.0` is not compatible with <a href="https://create-react-app.dev/">Create React App</a> <code>v5.0.0</code> or above due to a known <a href="https://github.com/Basis-Theory/basis-theory-js/issues/365#issuecomment-1662883062">issue</a>.
</Alert>
...
const bt = await new BasisTheory().init("<API Key>", { elements: true });
// use Elements
```
...
<script>
  ...
  window.addEventListener("load", async () => {
    try {
      ...
      await BasisTheory.init("<API Key>", { elements: true });
      // use Elements
    } catch (e) {
