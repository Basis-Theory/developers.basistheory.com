...
|-------------------|----------|----------|-----------------------------------------------------------------------|
| `token_id`        | false    | _string_ | The Basis Theory token id for the card token to be used               |
| `token_intent_id` | false    | _string_ | The Basis Theory token id for the card token intent to be used        |
| `pan`             | false    | _string_ | <Deprecated/> The Basis Theory token id for the card token to be used |

Either `token_id` or `token_intent_id` is required.
