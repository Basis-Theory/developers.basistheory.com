...
<Alert type={Alerts.WARNING}>Sessions [cannot be authorized](/docs/api/applications/sessions#authorize-session) directly from the frontend. All session authorization must be performed by a Private Application on the backend. This security measure ensures that sensitive operations are executed in a controlled environment, reducing exposure to potential threats.</Alert>
