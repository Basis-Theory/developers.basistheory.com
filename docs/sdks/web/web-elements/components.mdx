...
Once you have [created](/docs/sdks/web/web-elements/lifecycle#create-element) and [mounted](/docs/sdks/web/web-elements/lifecycle#mount-element) an element, you can invoke the following methods to interact with it:
...
| `blur()` | _void_ | All | Blurs (unfocuses) the element input |
...
| `setValue(tokenData)` | _void_ | All | Sets the element's value from a token reference (for displaying tokenized data) |
...
The `format()` method for card expiration dates implements Luxon's [toFormat](https://moment.github.io/luxon/api-docs/index.html#datetimetoformat) method, supporting all of Luxon's [formatting tokens](https://moment.github.io/luxon/#/formatting?id=table-of-tokens):
...
The `setValueRef()` method allows you to synchronize one element's value with another:
...
- Incomplete inputs will trigger a validation error during [tokenization](/docs/sdks/web/web-elements/services#tokenization-services)
- The `change` event includes a `complete` property that indicates whether the full mask pattern has been satisfied
- The displayed value (with formatting) will be used during tokenization unless a [transform](/docs/sdks/web/web-elements/options#transform) is applied


## Transform

The `transform` attribute allows you to modify user input before [tokenization](/docs/sdks/web/web-elements/services#tokenization-services), enabling you to store data in a standardized format regardless of how it's displayed to users.
...
The transform operation happens automatically during the [tokenization](/docs/sdks/web/web-elements/services#tokenization-services) process, ensuring that your stored data is in a clean, consistent format while maintaining a user-friendly display format.
