...
<Alert type={Alerts.INFO}>
  Need help? If you can't resolve your issue using this guide, our support team is ready to help at <a href="mailto:support@basistheory.com">support@basistheory.com</a>. See the <a href="#environment-information-to-provide-when-seeking-support">Getting Support</a> section for details on what information to provide.
</Alert>
...
Elements uses a secure multi-layered architecture where user input happens in iframes that are isolated from your main application. When you request data from these iframes (like during tokenization), the system must:

1. Securely establish a trusted channel between frames
2. Validate the request's origin and permissions
3. Coordinate data retrieval across multiple isolated iframes
4. Package and encrypt the values
5. Transfer the data across domain boundaries using postMessage
6. Verify the data's integrity before proceeding

This complex secure communication can fail in various environments for several reasons:

**Solutions:**

1. **Understand the Internal Retry Mechanism**

   Elements implement progressive backoff for value resolution failures:
