...
```javascript
{
  ...
  maskSatisfied: false,   // Whether the mask, if provided, is satisfied
  valid: false,            // Whether the current input is valid
  binInfo: BinInfo        // BIN information (only available if binLookup is enabled and 6 digits of the card are entered)
  selectedNetwork: string // Selected network (only available if coBadgedSupport is enabled)
}
```
[Bin Info](/docs/api/enrichments/card-details#response)
