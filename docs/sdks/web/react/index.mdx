...
<Alert type={Alerts.WARNING}>
  The Basis Theory JS React Elements SDK `>1.12.1` is not compatible with <a href="https://create-react-app.dev/">Create React App</a> <code>v5.0.0</code> or above
  due to a known <a href="https://github.com/Basis-Theory/basis-theory-js/issues/365#issuecomment-1662883062">issue</a>.
</Alert>
...
const App = () => {
  ...
  const { bt, error } = useBasisTheory("<API Key>", { elements: true });
