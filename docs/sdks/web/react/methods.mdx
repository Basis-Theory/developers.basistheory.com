...
const MyForm = () => {
  const { bt } = useBasisTheory("test_1234567890", { elements: true }); // replace test_... w/ your public API key
...
const MyForm = () => {
  ...
  const submit = async () => {
    ...
    try {
      const token = await bt.tokens.update("ca9f3fd7-3906-4087-83aa-9a6129221297", { // replace w/ desired token id
        data: {
          name: fullName,
        },
      });
...
All proxy calls take the same [options](/docs/sdks/web/react/methods#proxyrequestoptions) object as a parameter.
...
BasisTheory.tokenize({
  ...
  if (error instanceof BasisTheoryValidationError) { // only applies to tokenization
    // check error details
  } else if (error instanceof BasisTheoryApiError) {
    // check error data or status
  }
