...
const MyForm = () => {
  const { bt } = useBasisTheory("<API Key>", { elements: true }); // replace <API Key> w/ your public API key
  ...
  const reveal = async () => {
    ...
    try {
      const token = await bt.tokens.retrieve("ca9f3fd7-3906-4087-83aa-9a6129221297", {
        apiKey: "<Session API Key>", // api key is required and should belong to a session
      });
