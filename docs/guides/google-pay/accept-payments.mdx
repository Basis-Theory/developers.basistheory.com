...
Remember that Token Intents are short-lived and expire after 24 hours by default. If you're wanting to use the payment data stored in a Token Intent for recurring transactions, for example, you can convert the Token Intent to a [Token](/docs/api/tokens/tokens#token-object) by calling the [create Token endpoint](/docs/api/tokens/token-intents#convert-token-intent-to-token).
...
We'll send the Basis Theory Token Intent to [Adyen](https://www.adyen.com/) to test charging a card. We'll be doing this from our backend by using a [Proxy](/docs/api/proxies/ephemeral-proxy) and a Private Application. Although, we are also able to create and authenticate a Basis Theory [Session](/docs/api/applications/sessions) and make this request from the frontend as well.
