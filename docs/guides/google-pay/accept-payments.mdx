...
Firstly, you will render the Google Pay button on your page using Google's SDKs. When the user clicks the button, Google Pay will present the payment sheet to the user, which will contain the payment information (e.g., shipping details, transaction amount, card number, expiration date, etc.).
...
2. Decryption of the payment data - The encrypted payment data is then sent to Basis Theory for decryption and storage of the sensitive cardholder information. Then Basis Theory returns a token intent that can be used to process the payment.
...
Now that we have access to the encrypted payment data, we can use our newly created public key to send the payment data to Basis Theory using the [Google Pay connections endpoint](/docs/api/orchestration/google-pay) in the `onGooglePaymentButtonClicked` function.
...
For this integration guide, we'll be using [Adyen](https://www.adyen.com/) to test charging a card. In Adyen, you can charge a card by creating a [payment](https://docs.adyen.com/api-explorer/Checkout/latest/post/payments). We'll use a Basis Theory [ephemeral Proxy](/docs/api/proxies/ephemeral-proxy) to send the sensitive payment data to Adyen.

```shell showLineNumbers title="Create a payment transaction"
