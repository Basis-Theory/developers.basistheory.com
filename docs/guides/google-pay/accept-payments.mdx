...
Now that we have access to the encrypted payment data, we'll create a public key and send the payment data to Basis Theory using the [Google Pay connections endpoint](/docs/api/orchestration/google-pay) in the `onGooglePaymentButtonClicked` function.
...
We'll send the Basis Theory Token Intent to [Adyen](https://www.adyen.com/) to test charging a card. We'll be doing this from our backend by using a [proxy](/docs/api/proxies/ephemeral-proxy) and a private API key. Although, we are also able to create and authenticate a Basis Theory [Session](/docs/api/applications/sessions) and make this request from the frontend as well.
...
In Adyen, you can charge a card by creating a [payment](https://docs.adyen.com/api-explorer/Checkout/latest/post/payments). We'll setup and invoke the [ephemeral Proxy](/docs/api/proxies/ephemeral-proxy) like the following.
