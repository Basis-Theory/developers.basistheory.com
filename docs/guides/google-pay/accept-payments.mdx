...
With Checkout, you can charge a card by creating a [payment](https://www.checkout.com/docs/payments/add-payment-methods/google-pay#Pay_with_a_pre-decrypted_Google_Pay_token). We'll setup and invoke the [ephemeral Proxy](/docs/api/proxies/ephemeral-proxy) on our backend.
...
```json showLineNumbers title="Create a payment transaction for a DPAN"
{
    "source": {
        ...
        "expiry_year": `{{ token_intent: ${tokenIntentId} | json: \"$.data\" | card_exp: \"YYYY\" }}`,
// highlight-start
        "eci": `{{ token_intent: ${tokenIntentId} | json: \"$.data.authentication.eciIndicator\" }}`,
        "cryptogram": `{{ token_intent: ${tokenIntentId} | json: \"$.data.authentication.threeds_cryptogram\" }}`
// highlight-end
    },
  ...
}
