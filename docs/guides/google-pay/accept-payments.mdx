...
Now that we have access to the encrypted payment data, we'll create a public key and send the payment data to Basis Theory using the [Google Pay connections endpoint](/docs/api/connections/google-pay) in the `onGooglePaymentButtonClicked` function.
