...
We will use Basis Theory [Ephemeral Proxy](/docs/api/proxies/invoke-ephemeral-proxies), a tool that transparently performs detokenization, to share the sensitive banking data with the Payments Processor or Acquirer API. To do this, we will formulate our HTTPS request as if we were directly connecting to the target endpoint, with the following variations:
