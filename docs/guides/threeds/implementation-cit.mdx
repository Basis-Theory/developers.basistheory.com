...
### Are co-badged cards supported?

Yes, co-badged cards are supported. The `additional_card_brands` property on session response will include an array of all brands the card identifies with.
You can then [choose](#optional-card-brand-selection) which brand's authentication rails to use during the authentication step.
