...
Lastly, if you perform transactions without the customer present at checkout, these are classified as Merchant-Initiated Transactions (MIT). For guidance on implementing 3DS MIT, please refer to the implementation guide [here](/docs/guides/threeds/implementation-mit).
...
The **Private Application** is used by your backend system to retrieve 3DS sessions.

- **Permissions Required:** `3ds:session:read`

[Click here to create the Private Application](https://portal.basistheory.com/applications/create?type=private&permissions=3ds%3Asession%3Aread)

***Important:** Save the generated API Key. You'll use it later in this guide.*

## Creating a CIT 3DS Session

Differently from the regular [CIT Implementation](/docs/guides/threeds/implementation-cit), the 3DS session with redirection is created from the backend instead of the client-side.
Of note, the `callback_urls` parameter and `authentication_request` are required when creating the session with redirection.

The code examples below demonstrate how to create sessions with redirection using our API and available SDKs.

TODO: EXAMPLE OF CREATING A SESSION WITH REDIRECTION - THE CODE BELOW IS NOT CORRECT - ITS FOR AUTHENTICATING A SESSION, THIS IS AN EXAMPLE BODY:
...
Below is the recommended minimum payload for `authentication_request` required for successful 3DS authentications in most scenarios:
...
The `callback_urls` parameter is a JSON object that contains the callback URLs for the 3DS session.
It is used to redirect the customer to the `success` or `failure` URL after the authentication process is completed by Basis Theory.
...
- The `success` URL is the URL to redirect the customer to if the authentication process is successful.
- The `failure` URL is the URL to redirect the customer to if the authentication process fails.

## Redirecting to the Authentication Page

The session creation response will include a `redirect_url` in the following format:

```
https://api.basistheory.com/3ds/redirect/<SESSION_ID>?apiKey=
```

- The `SESSION_ID` is the ID of the session you created.
- The `apiKey` query parameter needs to be set to the `PUBLIC_API_KEY` you used to create the session.

This URL will be used to redirect the customer to the Basis Theory 3DS authentication page, either in place or in a popup window, where they will follow the necessary steps and complete a challenge if required.

```javascript showLineNumbers
// highlight-start
// open a popup window to the redirect URL
const popup = window.open(redirectUrl, '3ds-redirect');
// highlight-end
```

Following the authentication completion, the customer will be redirected to the `callback_urls.success` or `callback_urls.failure` parameter you provided when [creating the session](/docs/guides/threeds/implementation-cit-redirect#creating-a-cit-3ds-session-with-redirection).

## Retrieving the 3DS Session

After the customer has completed the authentication process and you have received a redirect to one of the `callback_urls`, you need to retrieve the session to verify the authentication results.

You can retrieve the session by making a GET request to the `/3ds/sessions/<SESSION_ID>` endpoint using your `PRIVATE_API_KEY`.
...
The `authentication` object returned in the retrieved session contains everything you need to successfully validate a 3DS transaction or identify why it failed.
...
If the `directory_status_code` indicates the code `C`, the cardholder completed an additional verification step—a "Challenge"—before the 3DS authentication can be finalized and the final authentication value (CAVV) provided.
