...
TODO: UPDATE THIS IMAGE TO INCLUDE THE REDIRECTION FLOW
...
The **Public Application** allows your front-end or client-side integration to create 3DS sessions securely and redirect the customer to the Basis Theory 3DS authentication page.
...
Of note, the `callback_urls` parameter and `authentication_request` are required when creating the session if you are using redirection.
...
Below is the recommended minimum payload for the `authentication_request` parameter required for successful 3DS authentications in most scenarios:
...
TODO: ADD IMAGE OF THE AUTHENTICATION PAGE
...
#### Option 1: Direct API Retrieval (Recommended)

Make a GET request to retrieve the session immediately after receiving the callback redirect:

```http
GET /3ds/sessions/<SESSION_ID>
```

Use your `PRIVATE_API_KEY` for this request, where `SESSION_ID` is the ID of the session you created.
...
- `3ds.session.authenticated` - Triggered when authentication finishes
- `3ds.session.challenge-completed` - Triggered when a challenge is completed

TODO: EXAMPLES OF RETRIEVING A SESSION
TODO: EXAMPLES OF HANDLING DIFFERENT SCENARIOS - CHALLENGE SHOULD GET CHALLENGE RESULT INSTEAD OF SESSION
