...
Differently from the rest, Stripe only returns `number` and `cvc` when retrieving an existing card. So you first must [create](https://stripe.com/docs/api/issuing/cards/create) the card directly calling Stripe, and then [retrieve](https://stripe.com/docs/api/issuing/cards/retrieve?lang=curl) it using the Proxy.
  </TabItem>
</Tabs>
