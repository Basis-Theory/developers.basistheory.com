...
You can also [create an application](/docs/api/applications#create-application) from your terminal:
...
Ensure you replace the `<MANAGEMENT_API_KEY>` with a Management Application API Key.
...
To install the SDK you can choose either our ES module or CDN hosted bundle through a `script` tag.
...
This step will add a [Text Element](/docs/sdks/web/javascript-elements/types#text-element) component to our page.
...
We will add a submit button for submitting the form and a `<div>` to hold the resulting token identifier.
