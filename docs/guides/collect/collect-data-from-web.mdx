...
Create a new directory for this guide called `collect-guide`:

```shell showLineNumbers
mkdir collect-guide && cd collect-guide
```

Within the directory, create two new files:

```shell showLineNumbers
touch index.html && touch index.js
```

Now we need to initialize a new npm project:

```shell showLineNumbers
npm init
```

Accept all of the default options and open the contents of the directory in a text editor or IDE of your choice.
...
We want to be able to tokenize the value within the Text Element without exposing it to our web application. We will add a submit button for submitting the form and a `<pre>` to hold the resulting token:
...
(async () => {
  bt = await new BasisTheory().init('key_3aYBJ6NbEvyU5FN9n1Xbvw', { elements: true });
