...
In the example above, we instruct Basis Theory to reveal both segments, without having to worry about the card brand or number length. [Click here to learn more about this filter](/docs/expressions/filters#card_mask).

### Aliasing

While storing or transmitting tokens between systems, you may encounter restrictive technical constraints that can render the default token Universally Unique Identifier (UUID) incompatible, to a point where only a custom algorithm for generating token IDs can be suitable. T

- Backwards compatibility, customization, alternative for masking

## Conclusion

By following the best practices prescribed in this guide, you can ensure that your user-facing applications are compliant with the latest security standards while protecting your users' sensitive data. The `token.id` obtained at the end of the [Storing Cards](#storing-cards) section is a synthetic replacement for the sensitive data and can be safely stored in your database, meeting compliance requirements and reducing the risk of exposure in case of data breaches.
