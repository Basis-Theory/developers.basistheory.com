...
Alternatively, if you are using JSON Web Tokens (JWT), you can verify its signature using a public key, before the Proxy starts processing the payload for tokenization. This avoids an extra network hop to your APIs in order to assert the API client authentication.
