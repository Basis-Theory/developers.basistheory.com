...
Next we'll setup our component to our liking. Let's [initiate](/docs/sdks/mobile/ios#initialization) our element, add a [mask](/docs/sdks/mobile/ios/options#mask), and add some [styling](/docs/sdks/mobile/ios#styling-elements) in our `SsnTextElement.swift` file
...
@objc(SsnElement) class SsnElement: RCTViewManager {
  ...
  override func view() -> UIView! {
    ...
    ssnTextElement.textAlignment = .center
...
Now let's add our function to tokenize. Notice in this function how we make use of the `resolve` and `rejector` params. These will get used to communicate the results of the tokenize function back to our Javascript.
...
Now it's time to mount `SsnTextElement` in Javascript. First, we'll go into the `App.tsx` file and setup the view where we'll display the secure text field.
...
Try typing in a social security number in the `ssnTextElement` and tap on Tokenize. Basis Theory's iOS SDK will pass the value for the element reference. This will
be securely submitted directly to the [Create Token Endpoint](/docs/api/tokens#create-token). The resulting [token](/docs/concepts/what-are-tokens) is printed to the app.
