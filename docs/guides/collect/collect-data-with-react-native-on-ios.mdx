...
Next we'll setup our component to our liking. Let's [initiate](/docs/sdks/mobile/ios#initialization) the TextElementUITextField, add a [mask](/docs/sdks/mobile/ios/options#mask), and add some [styling](/docs/sdks/mobile/ios#styling-elements) in our `SsnTextElement.swift` file
...
Now let's make sure we can tokenize data entered into our `ssnTextElement`. 

In this function, we make use of the `resolve` and `rejector` params. These will are used to communicate the results of the tokenize function back to our Javascript.
