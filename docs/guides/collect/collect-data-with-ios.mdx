...
Now we need to add a Basis Theory element component to our app. We will add the [TextElementUITextField](/docs/sdks/mobile/ios/types#textelementuitextfield) component.

The easiest way to add one of iOS elements is to first drop a `UITextField` onto your `Main.storyboard` file. To this open the `Main.storyboard` file, and click on plus button on the top right and search for "UITextField" in
...
First we need to add an outlet to our `ViewController.swift` so that we're able to manipulate the field programmatically. To do this, ensure your `Main.storyboard` file is open and click on the "Add Editor to the Right" button
on the top right. This will open the `Main.storyboard` on a new pane on the right.

Now in the Project Navigator on the left, click on the `ViewController.swift` file. This should replace the `Main.storyboard` file on the right pane.

Next, we control-drag from the `TextElementUITextField` on `Main.storyboard` to the top of our `ViewController` class in the `ViewController.swift` file on the right. In the window that appears, type in "ssnTextElement" for the
Name and click Connect. Xcode should suggest you import BasisTheoryElements, so let's go ahead and do that now.

You should have something similar to the following after these steps.
...
As you can see above, we take advantage of the existing `UITextField` API to style our component. All of our iOS elements extend `UITextField` to take advantage of the existing API's for ease of use and speed to production.

## Add a Mask to Text Element

Let's add some masking to format the input as the user types in the text field. The following sets a mask that looks like a social security number (ie. ###-##-####).
...
and drop the Filled Button component anywhere under the Text Field. Let's change the name of this button to "Tokenize" by double-clicking on the new UIButton.

Now we need to add an action for this new button. Follow the steps from [Style Your Form Components](#style-your-form-components) to add an action to the new UIButton in the `ViewController.swift` file. When control-dragging
the UIButton into `ViewController.swift` choose Action for the Connection and "tokenize" for the Name.
...
Replace "YOUR PUBLIC BT API KEY HERE" with the public API key created from the [Getting Started](#getting-started) step.
...
Try typing in a fake social security number in the field and tap on Tokenize. Basis Theory's iOS SDK will pass the value for the element reference. This will be securely submitted directly to the
[Create Token Endpoint](/docs/api/tokens#create-token). The resulting [token](/docs/concepts/what-are-tokens) is printed to the Xcode console.
