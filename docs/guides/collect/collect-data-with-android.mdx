...
This guide will show you how to collect data in an Android application without touching the data. We will collect card numbers.
...
We will need to install Basis Theory's [Android Elements SDK](/docs/sdks/mobile/android), which will render a secure
[EditText](https://developer.android.com/reference/android/widget/EditText) View for capturing the data.
...
Now we need to add [CardNumberElement](/docs/sdks/mobile/android/types#cardnumberelement) component to our application.
...
We want to be able to tokenize the value within `CardNumberElement` without exposing it to our Android application.
