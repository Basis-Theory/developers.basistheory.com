...
Lithic expects an encrypted PIN Block in the `pin` field when updating a card. [See documentation](https://docs.lithic.com/docs/cards#encrypted-pin-block-enterprise).
...
Marqeta accepts the `pin` in plaintext along with a `control_token` that you can obtain going directly to them. [See documentation](https://www.marqeta.com/docs/core-api/pins#putPins).
...
async function submit () {
  try {
    ...
    await bt.proxy.patch({
      // card id concatenated to the proxy destination_url
