...
<Alert>
  Be sure to replace <code>&lt;Management API Key></code> with the Management API Key you created previously.
</Alert>
...
async function init() {
  bt = await new BasisTheory().init('<Public API Key>', { elements: true });
...
class ViewController: UIViewController {
    ...
    @IBAction func submit(_ sender: Any) {
        BasisTheoryElements.createSession(apiKey: "<Public API Key>") { data, error in
...
<Alert>
  Be sure to replace <code>&lt;Public API Key></code> with the Public API Key you created previously.
</Alert>
...
namespace server.Controllers
{
  ...
  {
    // highlight-start
    var client = new SessionClient("<Private API Key>");
...
<Alert>
  Be sure to replace <code>&lt;Private API Key></code> with the Private API Key you created previously.
</Alert>
