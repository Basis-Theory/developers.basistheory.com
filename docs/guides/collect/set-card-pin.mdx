...
In this guide we will cover the necessary steps to integrate your frontend with Basis Theory SDKs to securely capture the card PIN via user input and pass it to the Basis Theory Proxy, which will inject Issuer authentication and connect to the Issuer API.
...
```
Notice how no permissions are assigned at this point. They will be programmatically granted later using [sessions](/docs/concepts/access-controls#what-are-sessions).
