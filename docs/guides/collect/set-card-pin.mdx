...
In this guide, we will leverage Basis Theory Elements to securely capture the card PIN via user input in the frontend, and pass it to Basis Theory Proxy, which will inject Issuer authentication and connect to the Issuer API.
...
You will need a [Public Application](/docs/api/applications#application-types) to initialize Basis Theory Elements with. This application represents your Frontend.
...
curl "https://api.basistheory.com/applications" \
  -X "POST" \
  -H "BT-API-KEY: test_mgt_1234567890" \
...
```
Notice how no permissions are assigned at this point. They will be programmatically granted later using sessions.

<Alert>
  Be sure to replace <code>test_mgt_1234567890</code> with the Management API Key you created previously.
</Alert>

<Alert>
  Save the <code>key</code> from the created Public Application as it will be used later in this guide.
</Alert>

### Private Application

Next you will need a [Private Application](/docs/api/applications#application-types) to authorize the Proxy call performed by your Frontend. This application represents your Backend.
...
<Alert>
  Save the <code>key</code> from the created Private Application as it will be used later in this guide.
</Alert>

### Pre-Configured Proxy

Now we will create a Proxy that listens for incoming requests from the Frontend, injects credentials and forwards the PIN to the Issuer API. To achieve that, we will leverage a [Request Transform](/docs/api/proxies/pre-configured-proxies#request-transforms) code to manipulate the request as necessary:

<Tabs className="bt-tabs" queryString="issuer">
  <TabItem value="lithic" label="Lithic">

Lithic expects an encrypted PIN Block in the `pin` field when updating a card. The code below receives the `pin` in plaintext, and encrypt the PIN Block using Lithic's Public Key, and injects the necessary credentials to call the endpoint. [See documentation](https://docs.lithic.com/docs/cards#encrypted-pin-block-enterprise).
...
async function init() {
  bt = await new BasisTheory().init('test_pub_1234567890', { elements: true });
...
export default function App() {
  const { bt } = useBasisTheory("test_pub_1234567890", {
    elements: true
  });
...
class ViewController: UIViewController {
    ...
    @IBAction func submit(_ sender: Any) {
        BasisTheoryElements.createSession(apiKey: "test_pub_1234567890") { data, error in
...
<Alert>
  Be sure to replace <code>test_pub_1234567890</code> with the Public API Key you created previously.
</Alert>
...
app.post("/authorize", async (request, response) => {
  const bt = await new BasisTheory().init("test_pvt_1234567890");
...
namespace server.Controllers
{
  ...
  {
    // highlight-start
    var client = new SessionClient("test_pvt_1234567890");

    [HttpPost]
    public ActionResult AuthorizeSession(IDictionary<string,string> body)
    {
      var authorizedSession = await client.AuthorizeAsync(new AuthorizeSessionRequest {
        Nonce = body["nonce"],
        Rules = new List<String> { "token:use" }
      });
...
def authorize_session():
  ...
  with basistheory.ApiClient(configuration=basistheory.Configuration(api_key="test_pvt_1234567890")) as api_client:
...
func authorizeSession(rw http.ResponseWriter, r *http.Request) {
  ...
  contextWithAPIKey := context.WithValue(context.Background(), basistheory.ContextAPIKeys, map[string]basistheory.APIKey{
    "ApiKey": {Key: "test_pvt_1234567890"},
  })
...
<Alert>
  Be sure to replace <code>test_pvt_1234567890</code> with the Private API Key you created previously.
</Alert>
...
With the user session properly authorized, it is time to use it to invoke the Pre-Configured Proxy, passing the Text Element value.


<Tabs className="bt-tabs" queryString="issuer">
  <TabItem value="lithic" label="Lithic">

Additionally, we pass the Lithic's card id in the request `path` to call the [Update Card endpoint](https://docs.lithic.com/docs/cards#update-card).
...
Additionally, we pass Marqeta's `control_token` for the PIN in the request `body`. You can create one by calling their [`/pins/controltoken`](https://www.marqeta.com/docs/core-api/pins#postPinsControltoken) endpoint.
