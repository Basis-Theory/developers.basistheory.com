...
The first step is we want to provide an input mask to our form. This will ensure that a phone number is properly formatted and complete. The Text Element component support an [input `mask`](/docs/sdks/elements/options#mask) property to be able to control the user input.
...
The Text Element component also supports a [`transform`](/docs/sdks/elements/options#transform) property, which will modify the input value prior to tokenizing it. We can update our previous example to the following:
