...
Once the `session` is created, it must be [authenticated](/docs/api/3ds/sessions#authenticate-session). In this process, the merchant or requestor must send to the 3DS server, and subsequently the card issuer, information about the transaction.

This is done by calling the Basis Theory backend endpoint `/3ds/sessions/{sessionId}/authenticate` from your own backend, with the private API key created earlier.
