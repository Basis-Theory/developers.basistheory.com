...
Run the following command in your terminal to search for tokens having both a metadata key `customer_type` with a value of `vip`
and a metadata key `spaceship` with a value of `X-Wing`:
