...
Basis Theory does not support the use of Apple's sandbox environment.
You will need to use a real card in your Apple Wallet.
Basis Theory will not return real card data for integrations using a test tenant.
Therefore, although the Apple Pay token you receive during development is real, the Basis Theory Token Intent will contain simulated data.
Only production tenants will return real, valid credit card tokens from Apple Pay tokens.
