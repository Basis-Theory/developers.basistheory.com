...
Basis Theory does not support Apple's sandbox environment. You must use a real card in your Apple [Wallet](https://support.apple.com/en-us/108398) for testing.
...
Adding the `apple-pay-sdk.js` script will enables the `<apple-pay-button>` directive to your website.

The `validateMerchant` function Basis Theory to initiate the Apple Pay session.
...
Let's modify the `session.onpaymentauthorized` event to call Basis Theory.
    ...
    <script>
        ...
        async function performTransaction(event, session) {
            try {
                ...
                // Call backend server to proxy the token intent to the payment processor
                // ...
            } catch (error) {
                session.completePayment(ApplePaySession.STATUS_FAILURE);
