...
sequenceDiagram
	...
	participant mfe as Merchant<br>Frontend
	participant msb as Merchant<br>Backend
	participant bta as Basis Theory<br>API
	participant psp as Processor<br>API

	activate cus
	cus ->>+ mfe: Click Apple "Pay" button
	mfe ->>+ bta: Create Apple Pay Session
	bta -->>- mfe: Apple Pay Session
	mfe -->> cus: Apple Payment Sheet
	cus ->> mfe: Authorize Payment
	mfe ->>+ bta: Tokenize Apple Pay DPAN

	bta -->>- mfe: Token Intent
	mfe -->> cus: Apple Pay<br>success notification
	deactivate cus

	alt Optionally Async
		mfe ->>+ msb: Process Payment
		msb ->>+ bta: [Proxy]<br>Process Payment<br>w/ {{ token intent }}
