...
We'll send the Basis Theory Apple Pay™ resource to a Payments Service Provider (PSP) to test charging a card.
We'll be doing this from our backend by using a [Proxy](/docs/api/proxies/invoke-ephemeral-proxies) and a Private Application.
...
We'll set up and invoke the [ephemeral Proxy](/docs/api/proxies/invoke-ephemeral-proxies) on our backend.
...
<Alert>You are not restricted to only using Checkout or Stripe. You can use any payment processor that accepts decrypted Apple Pay™ resources.</Alert>
...
Although the Apple Pay™ resource generated during development is authentic, the associated Basis Theory Apple Pay™ resource will contain simulated data only.

To access genuine, valid network tokens derived from Apple Pay™ resources, a **Production tenant** must be utilized.
