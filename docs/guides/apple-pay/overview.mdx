...
- **Acquirer**: bank or bank-sponsored entity
...
1. When a user chooses Apple Pay to pay for a good or service:
...
sequenceDiagram
    actor u as User
    participant aw as Apple Wallet
    participant mt as Merchant<br>Terminal
    participant me as Merchant<br>E-commerce
    participant ab as Acquirer
    participant cn as Card Network <br> Token Service Provider
    participant ib as Issuer
    ...
    alt In-person Payment
        aw ->> mt: DPAN, Dynamic CVV
        mt ->> ab: DPAN, Dynamic CVV, Transaction details
    else Online Payment
        aw ->>+ me: DPAN, Online Cryptogram
        me ->> ab: DPAN, Online Cryptogram, Transaction details
    end
    aw -->> u: OK ✅
    deactivate u

    ab ->>+ cn: DPAN, Authentication Data, Transaction details
    cn ->> cn: Detokenize FPAN
    cn ->>+ ib: FPAN, Authentication Data, Transaction details
    ib -->>- cn: Authorization Decision
    cn -->>- ab: Authorization Decision

    alt In-person Payment
        ab -->> mt: Authorization Decision
        deactivate ab
    else Online Payment
        ab -->> me: Authorization Decision
    end
```
