...
<SdkList sdks={['javascript', 'react', 'ios', 'react-native']} />
...
- Increase the `expires_at` to allow longer session, giving more time for the user to authorize the payment
- The Apple Pay Sheet times out after 30 seconds. If the whole APP > BT > PSP flow takes more than that, process the payment asynchronously.
