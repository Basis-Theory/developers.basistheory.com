...
2. **Decryption of the payment data** - After the user authorizes the payment, the encrypted payment data is sent to Basis Theory for decryption and storage.
   Basis Theory responds with an [Apple Pay™](/docs/api/apple-pay/api) resource that can be used to process the payment.

3. **Processing of the payment data** - The Basis Theory Apple Pay™ resource is forwarded to the payment processor via Basis Theory Proxy, which translates Token back to raw data before sending the request.
