...
<Alert>Save the API Key from the created Private Application as it will be used later in this guide to capture the data and send it to a third-party.</Alert>
...
<Alert type={Alerts.INFO}>It is best practice to securely collect your data from a <a href="/docs/guides/collect/collect-data-from-web">web form</a> or <a href="/docs/guides/collect/collect-inbound-sensitive-data">API</a> to avoid security and compliance risks associated with directly handling the sensitive data.</Alert>

## Send the Data

We can leverage the Basis Theory [Ephemeral Proxy](/docs/api/proxies/invoke-ephemeral-proxies) to detokenize the stored data before forwarding it to a third-party. To do this, we will utilize [Expressions](/docs/expressions), which are based on the [Liquid template language](https://shopify.github.io/liquid).
...
<Alert>Be sure to replace <code>&lt;API_KEY></code> with the Private API Key you created in the <a href="#getting-started">Getting Started</a> step and replace <code>d2411dd4-17f0-47ce-a4e2-78a66f2e4606</code> with the token <code>id</code> you created in the <a href="#create-a-token">Create a Token</a> step.</Alert>
