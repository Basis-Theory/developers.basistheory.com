...
export default function App() {
  ...
  return (
    <BasisTheoryProvider bt={bt}>
      // highlight-start
      <CardNumberElement id="myCardNumber" ref={cardNumberRef} />
      // highlight-end
      <div style={{ display: "flex" }}>
        <div style={{ width: "100%" }}>
          // highlight-start
          <CardExpirationDateElement id="myCardExpiration" ref={cardExpirationRef} />
          // highlight-end
        </div>
        <div style={{ width: "100%" }}>
          // highlight-start
          <CardVerificationCodeElement id="myCardVerification" ref={cardVerificationRef} />
          // highlight-end
        </div>
      </div>
    </BasisTheoryProvider>
...
import { CardNumberElement, CardExpirationDateElement, CardVerificationCodeElement } from "./src";
...
<Alert>
  Be sure to replace <code>&lt;API_KEY></code> with the Public API Key you created previously.
</Alert>
...
const App = (): JSX.Element => {
  ...
  const display = async () => {
    try {
      const session = await BasisTheoryElements.createSession("<PUBLIC_API_KEY>");
    } catch (error) {
...
const App = (): JSX.Element => {
  ...
  const display = async () => {
    try {
      ...
      const token = await BasisTheoryElements.getTokenById(tokenId, session.sessionKey);
