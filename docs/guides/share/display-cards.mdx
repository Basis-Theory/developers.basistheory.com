...
Next, you will need a [Private Application](/docs/api/applications) using our PCI-compliant template `Read PCI Tokens`.
...
def authorize_session():
  ...
  with basistheory.ApiClient(configuration=basistheory.Configuration(api_key="test_priv_1234567890")) as api_client:
...
func authorizeSession(rw http.ResponseWriter, r *http.Request) {
  ...
  contextWithAPIKey := context.WithValue(context.Background(), basistheory.ContextAPIKeys, map[string]basistheory.APIKey{
    "ApiKey": {Key: "test_priv_1234567890"},
  })
...
<Alert>
  Be sure to replace <code>test_priv_1234567890</code> with the Private API Key you created previously.
</Alert>
...
class ViewController: UIViewController {
    ...
    @IBAction func display(_ sender: Any) {
        BasisTheoryElements.createSession(apiKey: "test_1234567890") { data, error in
            ...
            self.authorizeSession(nonce: nonce) { result, error in
                // highlight-start
                BasisTheoryElements.getTokenById(id: 'd2cbc1b4-5c3a-45a3-9ee2-392a1c475ab4', apiKey: sessionKey) { data, error in
...
class MainActivity : AppCompatActivity() {
    ...
    private fun display() {
      ...
      cardNumberElement.setValueRef(token.data.number)
      cardExpirationDateElement.setValueRef(token.data.expiration_month, token.data.expiration_year)
      cardVerificationCodeElement.setValueRef(token.data.cvc)
      // highlight-end
    }
...
In the steps above, we had retrieved the card token data within the secure Elements context. This means we can set Elements value, and therefore display it, but not touch interact with it directly.
...
Card tokens retain verification code for 1 hour. This limitation is enforced by default to comply with PCI-DSS guidelines. However, depending on your business case, extending this period may be possible upon analysis. Please, [reach out to our support team](https://basistheory.com/contact) for assistance anytime.
