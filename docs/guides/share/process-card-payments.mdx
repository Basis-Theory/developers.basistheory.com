...
Storing your customers' cards with Basis Theory platform grants you the ability to seamlessly shift between payment processors and give you competitive advantages including payment flexibility, reliability, cost savings, increased acceptance rates and future-proofing your business in case of processor shutdown.

This guide will explore how to forward stored cardholder data to any third party API endpoint, with minimal configuration requirements. By following the steps presented below, you are substantially de-scoping your servers and database from PCI DSS compliance. If you want to learn more how we can help you meet up to 95% of the PCI requirements, or if you need help filling out your SAQ, [reach out to our team](https://basistheory.com/contact)!
...
We will use Basis Theory [Ephemeral Proxy](/docs/api/proxies/ephemeral-proxy) to detokenize the cardholder data internally and share to the Payments Processor or Acquirer API. To do this, we will formulate our HTTPS request as if we were directly connecting to the target endpoint, with the following variations:
...
Some acquirers only accept inbound connections from whitelisted IP addresses. [Here](/docs/api/ip-addresses) you can find our full list. Basis Theory can also provide dedicated IPs for the more restrictive integrations. [Contact us](https://basistheory.com/contact) if you are interested.
...
During testing phase, make sure to create tokens using test cards documented by your payment processor, following the desired test scenario. Passing incorrect cards to test/sandbox endpoints may lead to rejected hard-to-debug transactions.

## Conclusion

By using this solution, you can confidently transmit cardholder data to Payment Processors via API requests without ever touching the sensitive information yourself. This approach not only improves security and reduces compliance risks but also provides the flexibility to establish your own relationships with Payment Processors.

With the Ephemeral Proxy, you can work with a broad range of processors and leverage the latest payment technologies, all while adhering to industry best practices. We hope this integration guide has been informative and helpful, and we welcome your feedback and questions as you continue to use our product.
