...
<QuestionCard question="Did you include a reference to the data storage at Basis Theory in your user terms of service?">
  <Content type="no">
    <Alert type="warning">Have your team add the following: "COMPANY uses Basis Theory, a third-party service provider for payment tokenization. Learn more about <a href="https://basistheory.com" target="_blank">Basis Theory</a>."</Alert>
  </Content>
</QuestionCard>
...
<QuestionCard question="Have you disabled Ephemeral Proxies in your Production Tenant(s)?" noType="right">
  <Content type="yes">
    <Alert type="info">Even though Ephemeral Proxies are production-ready, some companies find it harder to manage and audit data egress when they are enabled, given the dynamic nature of their [destination URLs](/docs/api/proxies/#destination-urls), and opt to use [Pre-Configured Proxies](/docs/api/proxies/pre-configured-proxies) instead.</Alert>
  </Content>
  <Content type="no">
    <Alert type="warning">TODO: Make sure the Proxy destination URLs are auditable and follow a change management process and have a change control aving it set upfront as part of the Proxy configuration reduces the risk and makes easier to audit destinations.</Alert>
  </Content>
  <Content type="na">
