...
<Alert>
  Save the API Key from the created Private Application as it will be used later
  in this guide.
</Alert>
...
<Alert>
  Be sure to replace <code>&lt;Private API Key></code> with the Private API Key
  you created in the{" "}
  <a href="#create-a-private-application">Create a Private Application</a> step.
</Alert>
...
const bt = await BasisTheory.init("<Public API Key>");
...
BasisTheoryElements.createSession("<Public API Key>") { data, error in
...
<Alert>
  Be sure to replace <code>&lt;Public API Key></code> with the Public API Key
  you created in the{" "}
  <a href="#create-a-public-application">Create a Public Application</a> step.
</Alert>
...
app.post("/authorize-session", async (request, response) => {
  if (!bt) {
    bt = await new BasisTheory().init("<Private API Key>");
  }
...
<Alert>
  ...
  you created in the{" "}
  <a href="#create-a-private-application">Create a Private Application</a> step,
  and <code>token_id_1234567890</code> with the id of the token created in the{" "}
  <a href="#create-a-token">Create a Token</a> step.
</Alert>
...
curl "https://api.basistheory.com/tokens/token_id_1234567890" \
  -H "BT-API-KEY: <Session API Key>"
```

<Alert>
  Be sure to replace <code>&lt;Session API Key></code> with the Session Key you
  received when <a href="#create-a-session">creating a session</a> and{" "}
  <code>token_id_1234567890</code> with the id of the token created in the{" "}
  <a href="#create-a-token">Create a Token</a> step.
</Alert>
...
<Alert>
  The code shown above retrieves the plaintext token data so that it is directly
  accessible within your frontend code. This will result in your frontend
  application being pulled into compliance scope!
</Alert>
