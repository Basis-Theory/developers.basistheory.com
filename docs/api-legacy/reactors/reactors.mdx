...
";
var createReactorRequest = new CreateReactorRequest("My Reactor", code)
{
...
var apiInstance = new ReactorsApi(config);
var id = new Guid("9fd32800-f0a6-4bc2-8335-4390faa61476");
var code = @"
      module.exports = async function (req) {
        // Do something with req.configuration.SERVICE_API_KEY
    ...
    ";
var updateReactorRequest = new UpdateReactorRequest("My Reactor", code: code)
{
  _Configuration = new Dictionary<string, string> {
    { "SERVICE_API_KEY", "key_abcd1234" }
  },
...
| `bt`            | A pre-configured Basis Theory JS instance for the [application](/docs/api/applications) defined with the Reactor. This will be `null` if no application was defined.. This has been deprecated, see  [Migrating from `bt` parameter in Reactors and Proxy Transforms](/docs/api/deprecations#migrating-from-bt-parameter-in-reactors-and-proxy-transforms) for more info. <Deprecated /> |
| `applicationOptions`            | An object capable of being used when initializing the [Basis Theory JS SDK](/docs/sdks/server-side/node) for the. `const client = new BasisTheoryClient(applicationOptions);` |
